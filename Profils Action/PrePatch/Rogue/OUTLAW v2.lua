    ...
    local function EnemyRotation(unit)
        ...
        if inCombat and Unit(unit):IsExists() then
            ...
            if A.BurstIsON(unit) and CanCast then
                ...
                if A.BloodoftheEnemy:AutoHeartOfAzeroth(unit, true) and Action.GetToggle(1, "HeartOfAzeroth") and A.BetweentheEyes:GetCooldown() == 0 and RtB_Buffs > 0 then
                    return A.BloodoftheEnemy:Show(icon)
                end
                ...
                if A.PurifyingBlast:AutoHeartOfAzeroth(unit, true) and A.BurstIsON(unit) and Action.GetToggle(1, "HeartOfAzeroth") and GetByRange(2, 10) >= 2 then
                ...
                if A.AdrenalineRush:IsReady("player") and A.BurstIsON(unit) and (Unit("player"):HasBuffs(A.AdrenalineRush.ID, true) == 0 and Player:EnergyTimeToMaxPredicted() > 1 ) then
                    -- Notification                    
			...
			then
                ...
                if A.BetweentheEyes:IsReadyByPassCastGCD(unit, nil, nil, true) and CanCast and (VarBteCondition) then
            ...
            if CanCast then
                ...
                if A.PistolShot:IsReady(unit) and Unit("player"):HasBuffs(A.Opportunity.ID, true) > 0 and Unit("player"):HasBuffs(A.DeadshotBuff.ID, true) > 0 then
                    return A.PistolShot:Show(icon)
                end
