...
A[3] = function(icon, isMulti)
	...
	--print(ImpsSpawnedDuring(2000))
        ...
        -- summon_pet
        if A.SummonPet:IsReady(unit) and IsLatenced(A.SummonPet) and not Pet:IsActive() then
        ...
        local function Implosion(unit)
            ...
            if A.SummonVilefiend:IsReady(player) and not isMoving and 
			(
			    (
				    A.SummonDemonicTyrant:GetCooldown() > 40 and MultiUnits:GetActiveEnemies() <= 2
				)
				or 
				A.SummonDemonicTyrant:GetCooldown() < 12
				or 
				not A.BurstIsON(unit)
			)
			then
        ...
        local function NetherPortalActive(unit)
            ...
            if A.SummonVilefiend:IsReady(player) and not isMoving and (A.SummonDemonicTyrant:GetCooldown() > 40 or A.SummonDemonicTyrant:GetCooldown() < 12) then
                return A.SummonVilefiend:Show(icon)
            end
        ...
        if inCombat and Unit(unit):IsExists() then
            ...
            if A.SummonVilefiend:IsReady(player) and not isMoving and 
			(
			    A.SummonDemonicTyrant:GetCooldown() > 40 
				or 
				A.SummonDemonicTyrant:GetCooldown() < 12
				or 
				not A.BurstIsON(unit)
