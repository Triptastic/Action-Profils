...
A[3] = function(icon, isMulti, isArena)
   ...
   -- end
    ...
    local function EnemyRotation(unit)
        ...
        local VarKSReadyCheck = A.KidneyShot:GetCooldown() == 0 and 
		                        A.KidneyShot:AbsentImun(unitID, Temp.TotalAndPhysAndStun) and 
								Unit(unitID):IsControlAble("stun", 0) and
                                (
								    A.ToxicBlade:IsSpellLearned() and A.ToxicBlade:GetCooldown() < 2 
									or 
    ...
    --        end			
        ...
        if Player:IsStealthed() and Unit(unit):HasDeBuffs("BreakAble") == 0 then
     ...
     --[[       if A.Garrote:IsReady(unit) and not A.LastPlayerCastName == A.Garrote:Info() and 
        ...
        -- garrote,refreshable
        if not Exsanguinated(unit, "Garrote") and A.Garrote:IsReady(unit) and 
        ...
        if not Exsanguinated(unit, "Garrote") and A.Garrote:IsReady(unit) and A.Garrote:AbsentImun(unit, Temp.TotalAndPhys) and CanCast and Unit(player):HasBuffs(A.SubterfugeBuff.ID, true) > 0 and not A.Exsanguinate:IsSpellLearned() and
        (
        ...
        if not Exsanguinated(unit, "Rupture") and A.Rupture:IsReady(unit) and A.Rupture:AbsentImun(unit, Temp.TotalAndPhys) and CanCast and Unit(player):HasBuffs(A.SubterfugeBuff.ID, true) > 0 and A.Exsanguinate:IsSpellLearned() and Unit(unit):HasDeBuffs(A.Rupture.ID, true) == 0 then
            return A.Rupture:Show(icon)
        end
   ...
   --         return A.Garrote:Show(icon)
    --    end
        ...
        if inCombat and A.BurstIsON(unit) and (not A.MasterAssassin:IsSpellLearned() or Unit(unit):HasDeBuffs(A.GarroteDebuff.ID, true) > 0) and Unit(unit):TimeToDie() > 5 then
     ...
     --[[       if A.Vanish:IsReady(unit) and 
			...
			-- marked_for_death,if=combo_points<3&target.time_to_die<10
            if A.MarkedForDeath:IsReadyByPassCastGCD(unit) and Unit(player):ComboPoints() < 3 and 
			(
			    VarKSReadyCheck 
				or
				Unit(unit):HasDeBuffs("Stuned") > 0 
				or 
		...
		-- garrote,if=talent.exsanguinate.enabled&!exsanguinated.garrote&dot.garrote.pmultiplier<=1&cooldown.exsanguinate.remains<2&spell_targets.fan_of_knives=1&raid_event.adds.in>6&dot.garrote.remains*0.5<target.time_to_die       
        if not Exsanguinated(unit, "Garrote") and not A.Exsanguinate:IsSpellLearned() and A.Garrote:IsReady(unit) and A.Garrote:AbsentImun(unit, Temp.TotalAndPhys) and CanCast and Player:ComboPoints() < CPMaxSpend() and 
        ...
        -- mutilate_fallback
        if A.Mutilate:IsReadyByPassCastGCD(unit) and A.Mutilate:AbsentImun(unit, Temp.TotalAndPhys) and CanCast and Player:ComboPoints() == 3 and A.ToxicBlade:GetCooldown() <= 5 and VarSingleTarget and
        ...
        if A.CrimsonTempest:IsReady(player) and A.CrimsonTempest:AbsentImun(unit, Temp.TotalAndPhys) and A.GetToggle(2, "AoE") and CanCast and InMelee(unit) and Player:ComboPoints() >= 4 and GetByRange(2, 10) and 
        ...
        -- rupture,if=talent.exsanguinate.enabled&!dot.garrote.refreshable&(combo_points>=cp_max_spend&cooldown.exsanguinate.remains<1&dot.rupture.remains*0.5<target.time_to_die)
        if not Exsanguinated(unit, "Rupture") and A.Rupture:IsReady(unit) and not A.Exsanguinate:IsSpellLearned() and A.Rupture:AbsentImun(unit, Temp.TotalAndPhys) and A.GetToggle(2, "AoE") and not VarSkipRupture and not VarSkipRupture and CanCast and Player:ComboPoints() >= 4 and 
        ...
        if not Exsanguinated(unit, "Rupture") and A.Rupture:IsReady(unit) and A.Exsanguinate:IsSpellLearned() and A.Rupture:AbsentImun(unit, Temp.TotalAndPhys) and CanCast and Unit(unit):HasDeBuffs(A.Garrote.ID, true) > 5.4 and
        ...
        if not Exsanguinated(unit, "Rupture") and A.Rupture:IsReady(unit) and not A.Exsanguinate:IsSpellLearned() and A.Rupture:AbsentImun(unit, Temp.TotalAndPhys) and Player:ComboPoints() >= 4 and CanCast and not VarSkipRupture and 
        ...
        -- crimson_tempest,target_if=min:remains,if=spell_targets>1&spell_targets<4&remains<2&combo_points>=4
        if A.CrimsonTempest:IsReady(player) and A.CrimsonTempest:AbsentImun(unit, Temp.TotalAndPhys) and A.GetToggle(2, "AoE") and CanCast and InMelee(unit) and Player:ComboPoints() >= 4 and 
        ...
        if A.ArcaneTorrent:AutoRacial(unit) and CanCast and Player:EnergyPredicted() + 15 < 70 and A.ToxicBlade:GetCooldown() <= 3 and Player:ComboPoints() >= 3 and 
        Unit(unit):HasDeBuffs(A.Vendetta.ID, true) > 0 then
        ...
        if A.Envenom:IsReadyByPassCastGCD(unit) and A.Envenom:AbsentImun(unit, Temp.TotalAndPhys) and Player:ComboPoints() >= 4 + num(A.DeeperStratagem:IsSpellLearned()) and CanCast 
		then
        ...
        if A.Mutilate:IsReadyByPassCastGCD(unit) and A.Mutilate:AbsentImun(unit, Temp.TotalAndPhys) and CanCast and Player:ComboPoints() < CPMaxSpend() and VarUseFiller then
            return A.Mutilate:Show(icon)
        end

        if A.PoisonedKnife:IsReady(unit) and A.PoisonedKnife:AbsentImun(unit, Temp.TotalAndPhys) and CanCast and not InMelee(unit) and inCombat and 
		(
		    Player:ComboPoints() < CPMaxSpend()
			or 
            Unit(unit):GetMaxSpeed() >= 100 and Unit(unit):HasDeBuffs("Slowed") == 0 and not Unit(unit):IsTotem()
		) 
		then
