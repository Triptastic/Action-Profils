...
Action[ACTION_CONST_HUNTER_BEASTMASTERY] = {
	...
	-- General Hunter Legendaries
	CalloftheWild                          = Action.Create({ Type = "Spell", ID = 336742  }),
    NessingwarysTrappingApparatus          = Action.Create({ Type = "Spell", ID = 336743  }),
    SoulforgeEmbers                        = Action.Create({ Type = "Spell", ID = 336745  }),
    CravenStrategem                        = Action.Create({ Type = "Spell", ID = 336747  }),
	-- Beastmastery Hunter Legendaries  
	DireCommand                            = Action.Create({ Type = "Spell", ID = 336819  }),
    FlamewakersCobraSting                  = Action.Create({ Type = "Spell", ID = 336822  }),
    QaplaEredunWarOrder                    = Action.Create({ Type = "Spell", ID = 336830  }),
    RylakstalkersPiercingFangs             = Action.Create({ Type = "Spell", ID = 336844  }),
	-- Oils
...
local function Interrupts(unit)
	...
	if castRemainsTime >= A.GetLatency() then
        -- CounterShot
        if useKick and
		--not notInterruptable and 
		A.CounterShot:IsReady(unit) 
		then 
...
local function PurgeDispellMagic(unit)
	...
	-- SpiritShock
	if A.SpiritShock:IsReady(unit) and CurrentCreatureFamily == LP.SPIRITBEAST and not ShouldStop and (Action.AuraIsValid(unit, "UseExpelEnrage", "Enrage") or Action.AuraIsValid(unit, "UseDispel", "Magic")) then
		return A.SpiritShock:Show(icon)
    end
			
    -- SonicBlast
    if A.SonicBlast:IsReady(unit) and CurrentCreatureFamily == LP.BAT and not ShouldStop and (Action.AuraIsValid(unit, "UseExpelEnrage", "Enrage") or Action.AuraIsValid(unit, "UseDispel", "Magic")) then
...
local function HandleAspectoftheWild()
    ...
    return     not Player:IsMounted() and not Unit("target"):IsDead() and ( 
        (A.BurstIsON(unit) and choice[1]) or 
        (GetByRange(2, 40) and choice[2]) or
        (A.AspectoftheWild:IsReadyByPassCastGCD(player) and choice[3])
    )	
end
...
A[3] = function(icon, isMulti)
	...
	if Unit(player):HasDeBuffs(A.GrandDelusionsDebuff.ID, true) > 0 and UseFeignDeathOnThingFromBeyond and A.FeignDeath:IsReady(player) then
	    StopAttack()
    ...
    local function EnemyRotation(unit)
        ...
        if inCombat and A.SurvivaloftheFittest:IsReady(player) and currentPetSpecName == LP.TENACITY and Unit(player):HasDeBuffs("Rooted") > 0 then
            return A.SurvivaloftheFittest:Show(icon)
        end

        -- MastersCall
        if inCombat and A.MastersCall:IsReady(player) and currentPetSpecName == LP.CUNNING and Unit(player):HasDeBuffs("Rooted") > 0 then
            return A.MastersCall:Show(icon)
        end
		
        -- MastersCall mouseover
        if inCombat and A.MastersCall:IsReady("mouseover") and currentPetSpecName == LP.CUNNING and Unit("mouseover"):HasDeBuffs("Rooted") > 0 and Unit("mouseover"):GetRange() < 40 then
            return A.MastersCall:Show(icon)
        end	
		...
		if BurstIsON(unit) and unit ~= "mouseover" and inCombat and not profileStop and CanCast  then
			...
			(
			    ...
			    (
				    Player:AreaTTD(12) >= BloodoftheEnemyAoETTD 
				    and 
				    GetByRange(BloodoftheEnemyUnits, 12)
                )
                or Unit(unit):IsBoss()				
			)
			then
        ...
        -- aspect_of_the_wild
        if A.AspectoftheWild:IsReadyByPassCastGCD(player) and combatTime > 2 and 
		(
        ...
        if A.BestialWrath:IsReady(player) and inCombat and
		(
		    Unit(player):HasBuffs(A.BestialWrathBuff.ID, true) < GetGCD()
			or
	    ...
	    then 
            ...
            if A.BestialWrath:IsReady(player) 
			--and ((BestialWrathTTD > 0 and Unit(unit):TimeToDie() >= BestialWrathTTD) or Unit(unit):TimeToDie() >= 0) and HandleBestialWrath() 
			and 
		    (
            ...
            if inCombat and A.BarbedShot:IsReadyByPassCastGCD(unit)  then
            ...
            -- aspect_of_the_wild
            if A.AspectoftheWild:IsReadyByPassCastGCD(player) and ((AspectoftheWildTTD > 0 and Unit(unit):TimeToDie() >= AspectoftheWildTTD) or Unit(unit):TimeToDie() >= 0) and HandleAspectoftheWild() and Unit(player):HasBuffs(A.AspectoftheWildBuff.ID, true) == 0 then
                return A.AspectoftheWild:Show(icon)
            end
	    ...
	    then 	
			...
			(
				...
				or
				Unit(player):HasBuffs(A.BestialWrathBuff.ID, true) == 0 and 
				(
				    (A.BurstIsON(unit) and A.AspectoftheWild:GetCooldown() > 15) 
					or 
            ...
            if inCombat and A.BarbedShot:IsReadyByPassCastGCD(unit) and 
			(
            ...
            if inCombat and A.AspectoftheWild:IsReadyByPassCastGCD(player) and ((AspectoftheWildTTD > 0 and Unit(unit):TimeToDie() >= AspectoftheWildTTD) or Unit(unit):TimeToDie() >= 0) and Unit(player):HasBuffs(A.AspectoftheWildBuff.ID, true) == 0 and HandleAspectoftheWild() and 
			(
            ...
            if inCombat and A.BarbedShot:IsReadyByPassCastGCD(unit) and 
			(
			    A.DanceofDeath:GetAzeriteRank() > 1 and 
				Unit(player):HasBuffs(A.DanceofDeathBuff.ID, true) < GetGCD() + GetCurrentGCD() 
				--and Player:CritChancePct() > 40
			) 
			then
                return A.BarbedShot:Show(icon)
            end
            ...
            if inCombat and A.BarbedShot:IsReadyByPassCastGCD(unit) and (Unit(pet):HasBuffs(A.FrenzyBuff.ID, true) - GetGCD() > A.BarbedShot:GetSpellChargesFullRechargeTime()) then
                return A.BarbedShot:Show(icon)
            end
        end
    end
