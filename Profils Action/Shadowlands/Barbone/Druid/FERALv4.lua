...
Action[ACTION_CONST_DRUID_FERAL] = {
	...
	ApexPredatorsCraving                    = Action.Create({ Type = "Spell", ID = 339139  }),
    CateyeCurio                             = Action.Create({ Type = "Spell", ID = 339144  }),
    EyeofFearfulSymmetry                    = Action.Create({ Type = "Spell", ID = 339141  }),
    Frenzyband                              = Action.Create({ Type = "Spell", ID = 340053  }),
	-- Oils
...
--print("Rake:" .. A.RakeDebuff:BaseDuration())
    ...
    local function EnemyRotation(unit)
        ...
        if combatTime == 0 and not Unit(unit):IsDead() and IsInCatForm and Unit(unit):GetRange() <= OpenerRange and not Player:IsMounted() and not profileStop and Unit(unit):IsExists() and unit ~= "mouseover" then
            ...
            -- berserk
            if A.Berserk:IsReadyByPassCastGCD(player) and Unit(player):HasBuffs(A.BerserkBuff.ID, true) == 0 and A.BurstIsON(unit) then
        ...
        if VarOpenerDone == 0 and Unit(unit):GetRange() <= OpenerRange and IsInCatForm and not Unit(unit):IsDead() then
            ...
            if A.Berserk:IsReadyByPassCastGCD(player) and A.TigersFury:GetCooldown() > 0 and A.BurstIsON(unit) then
                return A.Berserk:Show(icon) 
            end
      ...
      --  end
	    ...
	    if A.IsInPvP then
	  		if A.Maim:IsReady(unit) and Unit(unit):IsControlAble("stun", 0) and Unit(unit):HasDeBuffs("Stuned") == 0 and 
			   (A.Berserk:IsReadyByPassCastGCD(player) or A.FeralFrenzy:IsReady(unit) or EnemyTeam("HEALER"):GetCC() >= A.GetGCD() * 3) and Unit(unit):HasBuffs("DeffBuffs") == 0 and Player:ComboPoints() > 3 then
        ...
        if inCombat and IsInCatForm and not Unit(unit):IsDead() and Unit(unit):GetRange() < 7 and unit ~= "mouseover" then
            ...
            if A.Berserk:IsReadyByPassCastGCD(player) and A.BurstIsON(unit) and 
			(
            ...
            if A.Incarnation:IsReadyByPassCastGCD(player) and A.BurstIsON(unit) and 
			(
			    EnergyPredicted >= 30 and 
				(
