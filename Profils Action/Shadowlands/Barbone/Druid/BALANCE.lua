...
Action[ACTION_CONST_DRUID_BALANCE] = {
	...
	BalanceofAllThings                      = Action.Create({ Type = "Spell", ID = 339942  }),
    OnethsClearVision                       = Action.Create({ Type = "Spell", ID = 338661  }),
    PrimordialArcanicPulsar                 = Action.Create({ Type = "Spell", ID = 338668  }),
    TimewornDreambinder                     = Action.Create({ Type = "Spell", ID = 339949  }),
	-- Oils
...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Precombat(unit)
			...
			and ((Pull > 0.1 and Pull <= A.SolarWrath:GetSpellCastTime() + A.GetGCD()) or not Action.GetToggle(1, "BossMods"))
			then
            ...
            and ((Pull > 0.1 and Pull <= A.SolarWrath:GetSpellCastTime()) or not Action.GetToggle(1, "BossMods"))			
			then
			...
			and ((Pull > 0.1 and Pull <= 0.5) or not Action.GetToggle(1, "BossMods"))
			then
        ...
        local function Burst(unit)	
			...
			if CaInc:IsReadyByPassCastGCD(player) and (Unit(unit):HasDeBuffs(A.StellarFlareDebuff.ID, true) > 6 or not A.StellarFlare:IsSpellLearned()) and (Unit(player):HasBuffs(A.MemoryofLucidDreams.ID, true) > 0 
