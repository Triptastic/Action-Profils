...
A[3] = function(icon, isMulti)
    ...
    local function HealingRotation(unit) 
        ...
        -- #9 Burst Cooldowns
        -- #9.1 HolyAvenger
        if A.HolyAvenger:IsReadyByPassCastGCD(player) and A.BurstIsON(unit) and A.HolyAvenger:IsSpellLearned() and
        -- AW Original
        ...
        -- #9.2 Avenging Wrath
        if A.AvengingWrath:IsReadyByPassCastGCD(player) and A.BurstIsON(unit) and Unit(player):HasBuffs({A.AvengingCrusader.ID, A.AvengingWrath.ID}, true) == 0 and
        (
            -- HEALING
            (
        ...
        (    
            (
                ...
                HealingEngine.HealingByRange((A.BreakingDawn:GetAzeriteRank() > 0 and 40) or 15, nil, A.LightofDawn) >= 3
            ) or
            -- Divine Purpose
            (
