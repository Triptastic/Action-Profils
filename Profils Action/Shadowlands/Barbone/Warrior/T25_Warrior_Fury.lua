...
local function RazorCoralIsApplied()
    return MultiUnits:GetByRangeAppliedDoTs(40, 5, A.RazorCoralDebuff.ID) > 0
end
    ...
    local function EnemyRotation(unit)
        ...
        local function Precombat_DBM(unit)            
            ...
            if Unit(unit):IsExists() then
                ...
                -- recklessness
                if A.Recklessness:IsReadyByPassCastGCD(player) and A.BurstIsON(unit) and InRange(unit) and not ShouldStop and Pull > 0.1 and Pull <= 0.7 then
        ...
        local function Precombat(unit)
            ...
            if Unit(unit):IsExists() then
                ...
                -- recklessness
                if A.Recklessness:IsReadyByPassCastGCD(player) and A.BurstIsON(unit) and not ShouldStop and InRange(unit) then
                    return A.Recklessness:Show(icon)
                end
        ...
        local function SingleTarget(unit)
            ...
            -- siegebreaker
            if A.Siegebreaker:IsReady(unit) and not ShouldStop and InRange(unit) and 
			(
        ...
        if not inCombat and Precombat_DBMRot and Action.GetToggle(1, "BossMods") and CanCast then 
            return true
        end
        
        -- Out of combat call non DBM precombat
        local PrecombatRot = Precombat(unit)
        if not inCombat and PrecombatRot and not Action.GetToggle(1, "BossMods") and CanCast then 
            return true
        end   
        ...
        if inCombat and Unit(unit):IsExists() then
		    ...
		    -- Piercing Howl
            if unit ~= "mouseover" and A.PiercingHowl:IsReady(unit) and 
			Unit(unit):HasDeBuffs(A.PiercingHowl.ID) <= 2 and 
			not Unit(unit):IsBoss() and 
			Unit(unit):GetRange() <= 14 and 
		    A.PiercingHowl:AbsentImun(unit, Temp.TotalAndPhys) and 
            ...
            if A.BloodoftheEnemy:AutoHeartOfAzerothP(unit, true) and A.Recklessness:GetCooldown() > 0 and 
			(
			    GetByRange(3, 12) 
				or 
				Unit(unit):IsBoss()
			)
			and not ShouldStop and 
			Action.GetToggle(1, "HeartOfAzeroth") and 
			--Unit(player):HasBuffs(A.Recklessness.ID, true) == 0 and 
			Unit(unit):HasDeBuffs(A.SiegebreakerDebuff.ID, true) == 0 
			then
            ...
            -- reaping_flames
            if A.ReapingFlames:AutoHeartOfAzerothP(unit, true) and not ShouldStop and Action.GetToggle(1, "HeartOfAzeroth") then
            ...
            if A.Recklessness:IsReadyByPassCastGCD(player) and A.BurstIsON(unit) and not ShouldStop and 
			(
			    InRange(unit) 
				or 
				A.LastPlayerCastName == A.Charge:Info()
			)
			then
                return A.Recklessness:Show(icon)
            end
            ...
            (
                ...
                (
					...
					or 
					A.GuardianofAzeroth:GetSpellTimeSinceLastCast() > 0 and A.GuardianofAzeroth:GetSpellTimeSinceLastCast() < 5
					or
					Unit(unit):HasDeBuffs(A.RazorCoralDebuff.ID, true) == 0 and not RazorCoralIsApplied()
				)
            )
            then
