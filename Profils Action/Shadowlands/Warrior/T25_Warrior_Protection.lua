...
local function Interrupts(unit)
	...
	if castRemainsTime >= A.GetLatency() then
        ...
        -- Stormbolt
        if useCC and A.Stormbolt:IsReady(unit) and A.Stormbolt:AbsentImun(unit, Temp.TotalAndPhysAndCC, true) and Unit(unit):IsControlAble("stun") then
            return A.Stormbolt              
        end  
    ...
    local function EnemyRotation(unit)
        ...
        if combatTime == 0 and unit ~= "mouseover" then
         ...
         --   end
            ...
            -- Intercept
            if A.Intercept:IsReady(unit) and not InRange(unit) and Unit(unit):GetRange() >= 8 and Unit(unit):GetRange() <= 20 and UseCharge 
		...
		if inCombat and unit ~= "mouseover" then
          ...
          --  end
            ...
            -- Shockwave
            if A.Shockwave:IsReady(player) and not Unit(unit):IsBoss() then
