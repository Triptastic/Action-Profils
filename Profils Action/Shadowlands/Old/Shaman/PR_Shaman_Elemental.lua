...
local function EvaluateCycleFlameShock30(unit)
    return Unit(unit):HasDeBuffsRefreshable(A.FlameShockDebuff.ID, true) and MultiUnits:GetByRangeInCombat(40, 5, 10) < 5
end

local function EvaluateCycleFlameShock53(unit)
    return Unit(unit):HasDeBuffsRefreshable(A.FlameShockDebuff.ID, true)
end

local function EvaluateCycleFlameShock68(unit)
    return not Unit(unit):HasDeBuffs(A.FlameShockDebuff.ID, true) or Unit(unit):HasDeBuffs(A.FlameShockDebuff.ID, true) <= GetGCD() and not Unit("player"):HasBuffs(A.SurgeofPowerBuff.ID, true)
end

local function EvaluateCycleFlameShock145(unit)
...
local function EvaluateCycleFlameShock214(unit)
...
local function EvaluateCycleFlameShock235(unit)
    return Unit(unit):HasDeBuffsRefreshable(A.FlameShockDebuff.ID, true)
end
...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Precombat(unit)
            ...
            if A.Stormkeeper:IsReady(unit) and Unit("player"):HasBuffsDown(A.StormkeeperBuff.ID, true) and (A.Stormkeeper:IsSpellLearned()) then
        ...
        local function Aoe(unit)
            ...
            -- flame_shock,target_if=refreshable&spell_targets.chain_lightning<5
            if A.FlameShock:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.FlameShock, 40, "min", EvaluateCycleFlameShock30) then
            ...
            -- fae_transfusion
            if A.FaeTransfusion:IsReady(unit) then
            ...
            if A.FlameShock:IsReady(unit) and isMoving then
                if Action.Utils.CastTargetIf(A.FlameShock, 40, "min", EvaluateCycleFlameShock53) then
                    return A.FlameShock:Show(icon) 
                end
            end
        ...
        local function SingleUnit(unit)(unit)
            ...
            -- flame_shock,target_if=!ticking|dot.flame_shock.remains<=gcd&!buff.surge_of_power.up
            if A.FlameShock:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.FlameShock, 40, "min", EvaluateCycleFlameShock68) then
                    return A.FlameShock:Show(icon) 
                end
            end
            -- fae_transfusion
            ...
            -- lightning_bolt,if=buff.stormkeeper.up&(buff.master_of_the_elements.up&!talent.surge_of_power.enabled|buff.surge_of_power.up)
            if A.LightningBolt:IsReady(unit) and (Unit("player"):HasBuffs(A.StormkeeperBuff.ID, true) and (Unit("player"):HasBuffs(A.MasteroftheElementsBuff.ID, true) and not A.SurgeofPower:IsSpellLearned() or Unit("player"):HasBuffs(A.SurgeofPowerBuff.ID, true))) then
                return A.LightningBolt:Show(icon)
            end
            
            -- earthquake,if=spell_targets.chain_lightning>1
            if A.Earthquake:IsReady(unit) and (MultiUnits:GetByRangeInCombat(40, 5, 10) > 1) then
                return A.Earthquake:Show(icon)
            end
            
            -- earth_shock,if=!buff.surge_of_power.up&talent.master_of_the_elements.enabled&buff.master_of_the_elements.up
            if A.EarthShock:IsReady(unit) and (not Unit("player"):HasBuffs(A.SurgeofPowerBuff.ID, true) and A.MasteroftheElements:IsSpellLearned() and Unit("player"):HasBuffs(A.MasteroftheElementsBuff.ID, true)) then
                return A.EarthShock:Show(icon)
            end
            
            -- lightning_bolt,if=cooldown.storm_elemental.remains>(cooldown.storm_elemental.duration-30)&talent.storm_elemental.enabled
            if A.LightningBolt:IsReady(unit) and (A.StormElemental:GetCooldown() > (A.StormElemental:BaseDuration() - 30) and A.StormElemental:IsSpellLearned()) then
                return A.LightningBolt:Show(icon)
            end
            ...
            if A.FlameShock:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.FlameShock, 40, "min", EvaluateCycleFlameShock145) then
                    return A.FlameShock:Show(icon) 
                end
            end
            ...
            if A.Icefury:IsReady(unit) and (A.Icefury:IsSpellLearned() and not (Player:Maelstrom() > 75 and A.LavaBurst:GetCooldown() <= 0)) then
            ...
            if A.FlameShock:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.FlameShock, 40, "min", EvaluateCycleFlameShock214) then
                    return A.FlameShock:Show(icon) 
                end
            end
            -- chain_lightning,if=!buff.stormkeeper.up&spell_targets.chain_lightning>1
            if A.ChainLightning:IsReady(unit) and (not Unit("player"):HasBuffs(A.StormkeeperBuff.ID, true) and MultiUnits:GetByRangeInCombat(40, 5, 10) > 1) then
                return A.ChainLightning:Show(icon)
            end
            ...
            -- flame_shock,moving=1,target_if=refreshable
            if A.FlameShock:IsReady(unit) and isMoving then
                if Action.Utils.CastTargetIf(A.FlameShock, 40, "min", EvaluateCycleFlameShock235) then
                    return A.FlameShock:Show(icon) 
                end
            end
        ...
        if inCombat and Unit(unit):IsExists() then
            ...
            if A.PotionofSpectralIntellect:IsReady(unit) and Potion and (expected_combat_length - Unit("player"):CombatTime() < 60) then
                return A.PotionofSpectralIntellect:Show(icon)
            end
            ...
            -- flame_shock,if=!ticking&spell_targets.chainlightning<4
            if A.FlameShock:IsReady(unit) and (not Unit(unit):HasDeBuffs(A.FlameShockDebuff.ID, true) and MultiUnits:GetByRangeInCombat(5, 5, 10) < 4) then
                return A.FlameShock:Show(icon)
            end
            
            -- use_items
            -- fire_elemental,if=!talent.storm_elemental.enabled&(expected_combat_length-time<32|expected_combat_length-time>155)
            if A.FireElemental:IsReady(unit) and A.BurstIsON(unit) and (not A.StormElemental:IsSpellLearned() and (expected_combat_length - Unit("player"):CombatTime() < 32 or expected_combat_length - Unit("player"):CombatTime() > 155)) then
                return A.FireElemental:Show(icon)
            end
            
            -- storm_elemental,if=talent.storm_elemental.enabled&(expected_combat_length-time<32|expected_combat_length-time>155)
            if A.StormElemental:IsReady(unit) and A.BurstIsON(unit) and (A.StormElemental:IsSpellLearned() and (expected_combat_length - Unit("player"):CombatTime() < 32 or expected_combat_length - Unit("player"):CombatTime() > 155)) then
