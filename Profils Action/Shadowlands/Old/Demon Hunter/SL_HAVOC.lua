...
Action[ACTION_CONST_DEMONHUNTER_HAVOC] = {
    ...
    ChaoticTransformation                  = Action.Create({ Type = "Spell", ID = 288754 }), -- BFA Azerite
    ...
    RevolvingBlades                        = Action.Create({ Type = "Spell", ID = 279581 }), -- BFA Azerite
    ...
    UnboundChaos                           = Action.Create({ Type = "Spell", ID = 275144}),
    UnboundChaosBuff                       = Action.Create({ Type = "Spell", ID = 337313}),
    ...
    SinfulBrandDebuff                      = Action.Create({ Type = "Spell", ID = 317009, Hidden = true     }),
    TheHunt                                = Action.Create({ Type = "Spell", ID = 323639}),
    -- Conduits
    ExposedWoundDebuff                     = Action.Create({ Type = "Spell", ID = 339229, Hidden = true     }), -- Triggered by Serrated Glaive
    SerratedGlaive                         = Action.Create({ Type = "Spell", ID = 339230}),
    -- Potions (BFA)
...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)     
        ...
        VarWaitingForEssenceBreak = A.EssenceBreak:IsSpellLearned() and (not bool(VarPoolingForBladeDance)) and (not bool(VarPoolingForMeta)) and A.EssenceBreak:GetCooldown() == 0        
        ...
        if BurstIsON(unit) and unit ~= "mouseover" and inCombat and not profileStop then
            ...
            if S.SinfulBrand:IsReady(unit) and Unit(unit):HasDeBuffs(A.SinfulBrandDebuff.ID, true) == 0 then
                return A.SinfulBrand:Show(icon)
            end
  
            -- the_hunt
            if S.TheHunt:IsReady(unit) then
                return A.TheHunt:Show(icon)
            end
  
            -- fodder_to_the_flame
            if S.FoddertotheFlame:IsReady(unit) then
                return A.FoddertotheFlame:Show(icon)
            
            end
   
            -- elysian_decree
            if S.ElysianDecree:IsReady(unit) then
            ...
            if Unit("player"):GetLevel() < 60 and Essences(unit) then
                return true
            end 
        ...
        -- concentrated_flame,if=(!dot.concentrated_flame_burn.ticking&!action.concentrated_flame.in_flight|full_recharge_time<gcd.max)
        if Unit("player"):GetLevel() < 60 and A.ConcentratedFlame:AutoHeartOfAzeroth(unit, true)  and UseHeartOfAzeroth 
		and (
		        (Unit(unit):HasDeBuffs(A.ConcentratedFlameBurn.ID, true) == 0 and not A.ConcentratedFlame:IsSpellInFlight() or A.ConcentratedFlame:GetSpellChargesFullRechargeTime() < A.GetGCD())
		    ) then
            return A.ConcentratedFlame:Show(icon)
		...
		-- reaping_flames
        if Unit("player"):GetLevel() < 60 and A.ReapingFlames:AutoHeartOfAzeroth(unit, true) and UseHeartOfAzeroth then
            return A.ReapingFlames:Show(icon)
        end
        ...
        if (A.EssenceBreak:IsSpellLearned() and (VarWaitingForEssenceBreak or Unit(unit):HasDeBuffs(A.EssenceBreakDebuff.ID, true) > 0)) then
		
            -- essence_break,if=fury>=80&(cooldown.blade_dance.ready|!variable.blade_dance)
            if A.DarkSlash:IsCastableP() and InMelee(unit) and (Player:Fury() >= 80 and (A.BladeDance:GetCooldown() == 0 or not VarBladeDance)) then
            ...
            if InMelee(unit) and (VarBladeDance and Unit(unit):HasDeBuffs(A.EssenceBreakDebuff.ID, true) > 0) then
                if A.DeathSweep:IsReady(unit) then
                    return A.DeathSweep:Show(icon)
                end
                if A.BladeDance:IsReady(unit) then
            ...
            if InMelee(unit) and Unit(unit):HasDeBuffs(A.EssenceBreakDebuff.ID, true) > 0 then
                if A.Annihilation:IsReady(unit) then
                    return A.Annihilation:Show(icon)
                end
                if A.ChaosStrike:IsReady(unit) then
        ...
        if A.Demonic:IsSpellLearned() and not profileStop and inCombat then
            ...
            if A.FelRush:IsReady(unit) and A.UnboundChaos:IsSpellLearned() and Unit("player"):HasBuffs(A.UnboundChaosBuff.ID, true) > 0 and A.FelRush:GetSpellCharges() == 2 then
            ...
            if A.GlaiveTempest:IsReady(unit) and not VarWaitingForMomentum and GetByRange(1, 8) then
            ...
            if A.ThrowGlaive:IsReady(unit) and (A.SerratedGlaive:IsSpellLearned() and A.EyeBeam:GetCooldown() < 6 and Unit("player"):HasBuffs(A.MetamorphosisBuff.ID, true) > 0 and Unit(unit):HasDeBuffs(A.ExposedWoundDebuff.ID, true) == 0) then
        ...
        if not A.Demonic:IsSpellLearned() and not profileStop and inCombat then
            ...
            (
                ...
                and not VarPoolingForBladeDance
				and not VarWaitingForEssenceBreak
            ) 
            then
                return A.Annihilation:Show(icon)
            end
            ...
            if A.ChaosStrike:IsReady(unit) and InMelee(unit) and 
			(
			    (
				    A.DemonBlades:IsAvailable() 
					or 
					not VarWaitingForMomentum 
					or 
					Player:FuryDeficit() < 30
				)
				and not VarPoolingForMeta and 
				not VarPoolingForBladeDance and 
				not VarWaitingForEssenceBreak
			)
			then
                return A.ChaosStrike:Show(icon)
            end
