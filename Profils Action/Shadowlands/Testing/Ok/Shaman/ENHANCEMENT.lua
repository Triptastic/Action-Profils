...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        if combatTime == 0 and not Unit(unit):IsDead() and Unit(unit):IsExists() and unit ~= "mouseover" then
            ...
            -- snapshot_stats			
            -- windfury_weapon
            if A.WindfuryWeapon:IsReady(unit) and ((Pull > 0.1 and Pull <= 5) or not DBM)
			then
                return A.WindfuryWeapon:Show(icon)
            end
            
            -- flametongue_weapon
            if A.FlametongueWeapon:IsReady(unit) and ((Pull > 0.1 and Pull <= 4) or not DBM)
			then
                return A.FlametongueWeapon:Show(icon)
            end
            
            -- lightning_shield
            if A.LightningShield:IsReady(unit) and ((Pull > 0.1 and Pull <= 3) or not DBM)
			then
                return A.LightningShield:Show(icon)
            end
            
            -- stormkeeper,if=talent.stormkeeper.enabled
            if A.Stormkeeper:IsReady(unit) and (A.Stormkeeper:IsSpellLearned()) and ((Pull > 0.1 and Pull <= 2) or not DBM)
			then
                return A.Stormkeeper:Show(icon)
            end
            
            -- windfury_totem
            if A.WindfuryTotem:IsReady(unit) and ((Pull > 0.1 and Pull <= 2) or not DBM) 
			then
                return A.WindfuryTotem:Show(icon)
            end
            
            -- potion
            if A.PotionofSpectralAgility:IsReady(unit) and Potion and ((Pull > 0.1 and Pull <= 1) or not DBM)
			then
