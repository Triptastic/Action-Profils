...
Action[ACTION_CONST_MAGE_FROST] = {
    ...
    FrozenDebuff                           = Create({ Type = "Spell", ID =  }),
...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Aoe(unit)
            ...
            if A.Flurry:IsReady(unit) and ((remaining_winters_chill == 0 or Unit(unit):HasDeBuffsDown(A.WintersChillDebuff.ID, true)) and (Player:PrevGCD(1, A.Ebonbolt) or Unit("player"):HasBuffsStacks(A.BrainFreezeBuff.ID, true) and Unit("player"):HasBuffsStacks(A.FingersofFrostBuff.ID, true) == 0)) then
            ...
            if A.IceLance:IsReady(unit) and (Unit("player"):HasBuffsStacks(A.FingersofFrostBuff.ID, true) or Unit(unit):HasDeBuffs(A.FrozenDebuff.ID, true) > A.IceLance:TravelTime() or remaining_winters_chill and Unit(unit):HasDeBuffs(A.WintersChillDebuff.ID, true) > A.IceLance:TravelTime()) then
            ...
            if A.FireBlast:IsReady(unit) and (runeforge.disciplinary_command.equipped and A.BuffDisciplinaryCommand.ID, true:GetCooldown() == 0 and Unit("player"):HasBuffsDown(A.DisciplinaryCommandFireBuff.ID, true)) then
        ...
        local function Movement(unit)
            ...
            if A.ArcaneExplosion:IsReady(unit) and (Player:ManaPercentageP() > 30 and MultiUnits:GetByRangeInCombat(40, 5, 10) >= 2) then
                return A.ArcaneExplosion:Show(icon)
            end
        ...
        local function St(unit)
            ...
            if A.Flurry:IsReady(unit) and ((remaining_winters_chill == 0 or Unit(unit):HasDeBuffsDown(A.WintersChillDebuff.ID, true)) and (Player:PrevGCD(1, A.Ebonbolt) or Unit("player"):HasBuffsStacks(A.BrainFreezeBuff.ID, true) and (Player:PrevGCD(1, A.RadiantSpark) or Player:PrevGCD(1, A.GlacialSpike) or Player:PrevGCD(1, A.Frostbolt) or (Unit(unit):HasDeBuffs(A.MirrorsofTormentDebuff.ID, true) or Unit("player"):HasBuffsStacks(A.ExpandedPotentialBuff.ID, true) or Unit("player"):HasBuffs(A.FreezingWindsBuff.ID, true)) and Unit("player"):HasBuffsStacks(A.FingersofFrostBuff.ID, true) == 0))) then
                return A.Flurry:Show(icon)
            end
            ...
            if A.Blizzard:IsReady(unit) and (Unit("player"):HasBuffs(A.FreezingRainBuff.ID, true) or MultiUnits:GetByRangeInCombat(35, 5, 10) >= 3 or MultiUnits:GetByRangeInCombat(35, 5, 10) >= 2 and not runeforge.cold_front.equipped) then
                return A.Blizzard:Show(icon)
            end
            
            -- ray_of_frost,if=remaining_winters_chill=1&debuff.winters_chill.remains
            if A.RayofFrost:IsReady(unit) and (remaining_winters_chill == 1 and Unit(unit):HasDeBuffs(A.WintersChillDebuff.ID, true)) then
                return A.RayofFrost:Show(icon)
            end
            
            -- glacial_spike,if=remaining_winters_chill&debuff.winters_chill.remains>cast_time+travel_time
            if A.GlacialSpike:IsReady(unit) and (remaining_winters_chill and Unit(unit):HasDeBuffs(A.WintersChillDebuff.ID, true) > A.GlacialSpike:GetSpellCastTime() + A.GlacialSpike:TravelTime()) then
            ...
            if A.IceLance:IsReady(unit) and (remaining_winters_chill and remaining_winters_chill > Unit("player"):HasBuffsStacks(A.FingersofFrostBuff.ID, true) and Unit(unit):HasDeBuffs(A.WintersChillDebuff.ID, true) > A.IceLance:TravelTime()) then
                return A.IceLance:Show(icon)
            end
            ...
            if A.RadiantSpark:IsReady(unit) and (Unit("player"):HasBuffs(A.FreezingWindsBuff.ID, true) and MultiUnits:GetByRangeInCombat(40, 5, 10) == 1) then
                return A.RadiantSpark:Show(icon)
            end
            
            -- ice_lance,if=buff.fingers_of_frost.react|debuff.frozen.remains>travel_time
            if A.IceLance:IsReady(unit) and (Unit("player"):HasBuffsStacks(A.FingersofFrostBuff.ID, true) or Unit(unit):HasDeBuffs(A.FrozenDebuff.ID, true) > A.IceLance:TravelTime()) then
                return A.IceLance:Show(icon)
            end
            ...
            if A.ArcaneExplosion:IsReady(unit) and (runeforge.disciplinary_command.equipped and A.BuffDisciplinaryCommand.ID, true:GetCooldown() == 0 and Unit("player"):HasBuffsDown(A.DisciplinaryCommandArcaneBuff.ID, true)) then
                return A.ArcaneExplosion:Show(icon)
            end
            ...
            if A.GlacialSpike:IsReady(unit) and (Unit("player"):HasBuffsStacks(A.BrainFreezeBuff.ID, true)) then
                return A.GlacialSpike:Show(icon)
            end
  ...
  if Precombat(unit) then
    return true
        ...
        if inCombat and Unit(unit):IsExists() then
            ...
            if (MultiUnits:GetByRangeInCombat(40, 5, 10) >= 5) then
                if Aoe(unit) then
                    return true
                end
            end
