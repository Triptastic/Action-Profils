...
local function EvaluateCycleSunfire52(unit)
    return (Unit(unit):HasDeBuffsRefreshable(A.SunfireDebuff.ID, true) and Unit(unit):TimeToDie() > 14 - MultiUnits:GetByRangeInCombat(40, 5, 10) + Unit(unit):HasDeBuffs(A.SunfireDebuff.ID, true)) and (ap_check and eclipse.in_any)
end

local function EvaluateCycleAdaptiveSwarm79(unit)
    return not ticking and not A.AdaptiveSwarmDamage:IsSpellInFlight() or Unit(unit):HasDeBuffsStacks(A.AdaptiveSwarmDamageDebuff.ID, true) < 3 and Unit(unit):HasDeBuffs(A.AdaptiveSwarmDamageDebuff.ID, true) < 3
end

local function EvaluateCycleMoonfire98(unit)
    return (Unit(unit):HasDeBuffsRefreshable(A.MoonfireDebuff.ID, true) and Unit(unit):TimeToDie() > (14 + (MultiUnits:GetByRangeInCombat(5, 5, 10) * 1.5)) / MultiUnits:GetByRangeInCombat(40, 5, 10) + Unit(unit):HasDeBuffs(A.MoonfireDebuff.ID, true)) and ((A.CaInc:GetCooldown() == 0 or MultiUnits:GetByRangeInCombat(5, 5, 10) < 3 or (eclipse.in_solar or eclipse.in_both or eclipse.in_lunar and not A.SouloftheForest:IsSpellLearned()) and (MultiUnits:GetByRangeInCombat(5, 5, 10) < 10 * (1 + num(A.TwinMoons:IsSpellLearned()))) and FutureAstralPower > 50 - Unit("player"):HasBuffs(A.StarfallBuff.ID, true) * 6) and not Unit("player"):HasBuffs(A.KindredEmpowermentEnergizeBuff.ID, true) and ap_check)
end

local function EvaluateCycleStellarFlare175(unit)
    return (Unit(unit):HasDeBuffsRefreshable(A.StellarFlareDebuff.ID, true) and Unit(unit):TimeToDie() > 15) and (MultiUnits:GetByRangeInCombat(5, 5, 10) < 4 and ap_check and (Unit("player"):HasBuffs(A.CaIncBuff.ID, true) > 10 or not Unit("player"):HasBuffs(A.CaIncBuff.ID, true)))
end

local function EvaluateCycleAdaptiveSwarm392(unit)
    return not Unit(unit):HasDeBuffs(A.AdaptiveSwarmDamageDebuff.ID, true) and not A.AdaptiveSwarmDamage:IsSpellInFlight() and (not Unit(unit):HasDeBuffs(A.AdaptiveSwarmHealDebuff.ID, true) or Unit(unit):HasDeBuffs(A.AdaptiveSwarmHealDebuff.ID, true) > 5) or Unit(unit):HasDeBuffsStacks(A.AdaptiveSwarmDamageDebuff.ID, true) < 3 and Unit(unit):HasDeBuffs(A.AdaptiveSwarmDamageDebuff.ID, true) < 3 and Unit(unit):HasDeBuffs(A.AdaptiveSwarmDamageDebuff.ID, true)
end

local function EvaluateCycleSunfire415(unit)
    return (Unit(unit):HasDeBuffsRefreshable(A.SunfireDebuff.ID, true) and Unit(unit):TimeToDie() > 16) and (ap_check and (VarCritnotup or (FutureAstralPower < 30 and not Unit("player"):HasBuffs(A.CaIncBuff.ID, true)) or A.CaInc:GetCooldown() == 0))
end

local function EvaluateCycleMoonfire434(unit)
    return (Unit(unit):HasDeBuffsRefreshable(A.MoonfireDebuff.ID, true) and Unit(unit):TimeToDie() > 13.5) and (ap_check and (VarCritnotup or (FutureAstralPower < 30 and not Unit("player"):HasBuffs(A.CaIncBuff.ID, true)) or A.CaInc:GetCooldown() == 0) and not Unit("player"):HasBuffs(A.KindredEmpowermentEnergizeBuff.ID, true))
end

local function EvaluateCycleStellarFlare455(unit)
    return (Unit(unit):HasDeBuffsRefreshable(A.StellarFlareDebuff.ID, true) and Unit(unit):TimeToDie() > 16 + Unit(unit):HasDeBuffs(A.StellarFlareDebuff.ID, true)) and (ap_check and (VarCritnotup or FutureAstralPower < 30 or A.CaInc:GetCooldown() == 0))
end

local function EvaluateCycleAdaptiveSwarm678(unit)
    return not Unit(unit):HasDeBuffs(A.AdaptiveSwarmDamageDebuff.ID, true) and not A.AdaptiveSwarmDamage:IsSpellInFlight() and (not Unit(unit):HasDeBuffs(A.AdaptiveSwarmHealDebuff.ID, true) or Unit(unit):HasDeBuffs(A.AdaptiveSwarmHealDebuff.ID, true) > 5) or Unit(unit):HasDeBuffsStacks(A.AdaptiveSwarmDamageDebuff.ID, true) < 3 and Unit(unit):HasDeBuffs(A.AdaptiveSwarmDamageDebuff.ID, true) < 3 and Unit(unit):HasDeBuffs(A.AdaptiveSwarmDamageDebuff.ID, true)
end

local function EvaluateCycleMoonfire701(unit)
    return (Unit(unit):HasDeBuffsRefreshable(A.MoonfireDebuff.ID, true) and Unit(unit):TimeToDie() > 12) and ((Unit("player"):HasBuffs(A.CaIncBuff.ID, true) > 5 and (Unit("player"):HasBuffs(A.RavenousFrenzyBuff.ID, true) > 5 or not Unit("player"):HasBuffs(A.RavenousFrenzyBuff.ID, true)) or not Unit("player"):HasBuffs(A.CaIncBuff.ID, true) or FutureAstralPower < 30) and (not Unit("player"):HasBuffs(A.KindredEmpowermentEnergizeBuff.ID, true) or FutureAstralPower < 30) and ap_check)
end

local function EvaluateCycleSunfire724(unit)
...
local function EvaluateCycleStellarFlare747(unit)
...
local function EvaluateCycleSunfire928(unit)
    return Unit(unit):HasDeBuffs(A.MoonfireDebuff.ID, true) > Unit(unit):HasDeBuffs(A.SunfireDebuff.ID, true)
end

local function EvaluateCycleMoonfire945(unit)
    return (Unit(unit):HasDeBuffsRefreshable(A.MoonfireDebuff.ID, true) and Unit(unit):TimeToDie() > 12) and ((Unit("player"):HasBuffs(A.CaIncBuff.ID, true) > 5 or not Unit("player"):HasBuffs(A.CaIncBuff.ID, true) or FutureAstralPower < 30) and ap_check)
end

local function EvaluateCycleSunfire962(unit)
    return (Unit(unit):HasDeBuffsRefreshable(A.SunfireDebuff.ID, true) and Unit(unit):TimeToDie() > 12) and ((Unit("player"):HasBuffs(A.CaIncBuff.ID, true) > 5 or not Unit("player"):HasBuffs(A.CaIncBuff.ID, true) or FutureAstralPower < 30) and ap_check)
end

local function EvaluateCycleStellarFlare979(unit)
    return (Unit(unit):HasDeBuffsRefreshable(A.StellarFlareDebuff.ID, true) and Unit(unit):TimeToDie() > 16) and ((Unit("player"):HasBuffs(A.CaIncBuff.ID, true) > 5 or not Unit("player"):HasBuffs(A.CaIncBuff.ID, true) or FutureAstralPower < 30) and ap_check)
end

local function EvaluateCycleAdaptiveSwarm1188(unit)
    return not Unit(unit):HasDeBuffs(A.AdaptiveSwarmDamageDebuff.ID, true) and not A.AdaptiveSwarmDamage:IsSpellInFlight() and (not Unit(unit):HasDeBuffs(A.AdaptiveSwarmHealDebuff.ID, true) or Unit(unit):HasDeBuffs(A.AdaptiveSwarmHealDebuff.ID, true) > 5) or Unit(unit):HasDeBuffsStacks(A.AdaptiveSwarmDamageDebuff.ID, true) < 3 and Unit(unit):HasDeBuffs(A.AdaptiveSwarmDamageDebuff.ID, true) < 3 and Unit(unit):HasDeBuffs(A.AdaptiveSwarmDamageDebuff.ID, true)
end

local function EvaluateCycleMoonfire1211(unit)
    return (Unit(unit):HasDeBuffsRefreshable(A.MoonfireDebuff.ID, true) and Unit(unit):TimeToDie() > 12) and ((Unit("player"):HasBuffs(A.CaIncBuff.ID, true) > 5 and (Unit("player"):HasBuffs(A.RavenousFrenzyBuff.ID, true) > 5 or not Unit("player"):HasBuffs(A.RavenousFrenzyBuff.ID, true)) or not Unit("player"):HasBuffs(A.CaIncBuff.ID, true) or FutureAstralPower < 30) and (not Unit("player"):HasBuffs(A.KindredEmpowermentEnergizeBuff.ID, true) or FutureAstralPower < 30) and ap_check)
end
    ...
    local function EnemyRotation(unit)
        ...
        local function Aoe(unit)
            ...
            if A.Sunfire:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Sunfire, 40, "min", EvaluateCycleSunfire52) then
            ...
            if A.AdaptiveSwarm:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.AdaptiveSwarm, 40, "min", EvaluateCycleAdaptiveSwarm79) then
            ...
            if A.Moonfire:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Moonfire, 40, "min", EvaluateCycleMoonfire98) then
            ...
            if A.StellarFlare:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.StellarFlare, 40, "min", EvaluateCycleStellarFlare175) then
        ...
        local function Boat(unit)
            ...
            if A.AdaptiveSwarm:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.AdaptiveSwarm, 40, "min", EvaluateCycleAdaptiveSwarm392) then
                    return A.AdaptiveSwarm:Show(icon) 
                end
            end
            ...
            if A.Moonfire:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Moonfire, 40, "min", EvaluateCycleMoonfire434) then
                    return A.Moonfire:Show(icon) 
                end
            end
            ...
            if A.StellarFlare:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.StellarFlare, 40, "min", EvaluateCycleStellarFlare455) then
                    return A.StellarFlare:Show(icon) 
                end
            end
            -- force_of_nature,if=ap_check
        ...
        local function Dreambinder(unit)
            ...
            -- adaptive_swarm,target_if=!dot.adaptive_swarm_damage.ticking&!action.adaptive_swarm_damage.in_flight&(!dot.adaptive_swarm_heal.ticking|dot.adaptive_swarm_heal.remains>5)|dot.adaptive_swarm_damage.stack<3&dot.adaptive_swarm_damage.remains<3&dot.adaptive_swarm_damage.ticking
            if A.AdaptiveSwarm:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.AdaptiveSwarm, 40, "min", EvaluateCycleAdaptiveSwarm678) then
                    return A.AdaptiveSwarm:Show(icon) 
                end
            end
            ...
            if A.Moonfire:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Moonfire, 40, "min", EvaluateCycleMoonfire701) then
                    return A.Moonfire:Show(icon) 
                end
            end
            ...
            if A.Sunfire:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Sunfire, 40, "min", EvaluateCycleSunfire724) then
                    return A.Sunfire:Show(icon) 
                end
            end
            ...
            if A.StellarFlare:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.StellarFlare, 40, "min", EvaluateCycleStellarFlare747) then
                    return A.StellarFlare:Show(icon) 
                end
            end
            -- force_of_nature,if=ap_check
        ...
        local function Fallthru(unit)
            ...
            if A.Sunfire:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Sunfire, 40, "min", EvaluateCycleSunfire928) then
                    return A.Sunfire:Show(icon) 
                end
            end
            -- moonfire
        ...
        local function PrepatchSt(unit)
            ...
            if A.Moonfire:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Moonfire, 40, "min", EvaluateCycleMoonfire945) then
                    return A.Moonfire:Show(icon) 
                end
            end
            -- sunfire,target_if=refreshable&target.time_to_die>12,if=(buff.ca_inc.remains>5|!buff.ca_inc.up|astral_power<30)&ap_check
            if A.Sunfire:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Sunfire, 40, "min", EvaluateCycleSunfire962) then
                    return A.Sunfire:Show(icon) 
                end
            end
            ...
            if A.StellarFlare:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.StellarFlare, 40, "min", EvaluateCycleStellarFlare979) then
                    return A.StellarFlare:Show(icon) 
                end
            end
            -- force_of_nature,if=ap_check
        ...
        local function St(unit)
            ...
            -- adaptive_swarm,target_if=!dot.adaptive_swarm_damage.ticking&!action.adaptive_swarm_damage.in_flight&(!dot.adaptive_swarm_heal.ticking|dot.adaptive_swarm_heal.remains>5)|dot.adaptive_swarm_damage.stack<3&dot.adaptive_swarm_damage.remains<3&dot.adaptive_swarm_damage.ticking
            if A.AdaptiveSwarm:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.AdaptiveSwarm, 40, "min", EvaluateCycleAdaptiveSwarm1188) then
                    return A.AdaptiveSwarm:Show(icon) 
                end
            end
            -- moonfire,target_if=refreshable&target.time_to_die>12,if=(buff.ca_inc.remains>5&(buff.ravenous_frenzy.remains>5|!buff.ravenous_frenzy.up)|!buff.ca_inc.up|astral_power<30)&(!buff.kindred_empowerment_energize.up|astral_power<30)&ap_check
            if A.Moonfire:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Moonfire, 40, "min", EvaluateCycleMoonfire1211) then
                    return A.Moonfire:Show(icon) 
                end
            end
            -- sunfire,target_if=refreshable&target.time_to_die>12,if=(buff.ca_inc.remains>5&(buff.ravenous_frenzy.remains>5|!buff.ravenous_frenzy.up)|!buff.ca_inc.up|astral_power<30)&(!buff.kindred_empowerment_energize.up|astral_power<30)&ap_check
            if A.Sunfire:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.Sunfire, 40, "min", EvaluateCycleSunfire1234) then
                    return A.Sunfire:Show(icon) 
                end
            end
            -- stellar_flare,target_if=refreshable&target.time_to_die>16,if=(buff.ca_inc.remains>5&(buff.ravenous_frenzy.remains>5|!buff.ravenous_frenzy.up)|!buff.ca_inc.up|astral_power<30)&(!buff.kindred_empowerment_energize.up|astral_power<30)&ap_check
            if A.StellarFlare:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.StellarFlare, 40, "min", EvaluateCycleStellarFlare1257) then
                    return A.StellarFlare:Show(icon) 
                end
            end
            -- force_of_nature,if=ap_check
