...
Action[ACTION_CONST_MONK_BREWMASTER] = {
    ...
    CelestialBrew                          = Create({ Type = "Spell", ID =  }),
    ...
    SpinningCraneKick                      = Create({ Type = "Spell", ID =  }),
...
A:CreateCovenantsFor(ACTION_CONST_MONK_BREWMASTER)  -- where PLAYERSPEC is Constance (example: ACTION_CONST_MONK_BM)
...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        if inCombat and Unit(unit):IsExists() then
            ...
            if A.PurifyingBrew:IsReady(unit) and (stagger.pct > (6 * (1 - (A.PurifyingBrew:GetSpellChargesFrac()))) and (stagger.last_tick_damage_1 > ((0.02 + 0.001 * (1 - A.PurifyingBrew:GetSpellChargesFrac())) * stagger.last_tick_damage_30))) then
                return A.PurifyingBrew:Show(icon)
            end
            
            -- black_ox_brew,if=cooldown.purifying_brew.charges_fractional<0.5
            if A.BlackOxBrew:IsReady(unit) and (A.PurifyingBrew:GetSpellChargesFrac() < 0.5) then
            ...
            if A.CelestialBrew:IsReady(unit) and (Unit("player"):HasBuffsDown(A.BlackoutComboBuff.ID, true) and incoming_damage_1999ms > (health.max * 0.1 + stagger.last_tick_damage_4) and Unit("player"):HasBuffsStacks(A.ElusiveBrawlerBuff.ID, true) < 2) then
            ...
            if A.TigerPalm:IsReady(unit) and ((1 or A.SpecialDelivery:IsSpellLearned()) and Unit("player"):HasBuffs(A.BlackoutComboBuff.ID, true)) then
                return A.TigerPalm:Show(icon)
            end
            ...
            if A.SpinningCraneKick:IsReady(unit) and (MultiUnits:GetByRangeInCombat(40, 5, 10) >= 3 and A.KegSmash:GetCooldown() > GetGCD() and (Player:EnergyPredicted() + (Player:EnergyRegen() * (A.KegSmash:GetCooldown() + GetGCD()))) >= 65) then
