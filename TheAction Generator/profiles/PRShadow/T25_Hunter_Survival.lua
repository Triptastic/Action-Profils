...
local function EvaluateTargetIfFilterKillCommand47(unit)
  return Unit(unit):DebuffRemainsP(A.BloodseekerDebuff.ID, true)
end

local function EvaluateTargetIfKillCommand64(unit)
...
local function EvaluateTargetIfFilterKillCommand126(unit)
  return Unit(unit):DebuffRemainsP(A.BloodseekerDebuff.ID, true)
end

local function EvaluateTargetIfKillCommand145(unit)
...
local function EvaluateTargetIfFilterKillCommand247(unit)
  return Unit(unit):DebuffRemainsP(A.BloodseekerDebuff.ID, true)
end

local function EvaluateTargetIfKillCommand264(unit)
...
local function EvaluateTargetIfFilterKillCommand300(unit)
  return Unit(unit):DebuffRemainsP(A.BloodseekerDebuff.ID, true)
end

local function EvaluateTargetIfKillCommand319(unit)
  return Player:Focus() + Unit("player"):FocusCastRegen(A.KillCommand:GetSpellCastTime) < Player:FocusMax() and (Unit("player"):HasBuffsStacks(A.MongooseFuryBuff.ID, true) < 5 or Player:Focus() < A.MongooseBite:GetSpellPowerCostCache())
end


local function EvaluateCycleCarveCdr542(unit)
    return (MultiUnits:GetByRangeInCombat(8, 5, 10) < 5) and (MultiUnits:GetByRangeInCombat(8, 5, 10) < 5)
end

local function EvaluateTargetIfFilterMongooseBite576(unit)
  return Unit(unit):TimeToDie()
end

local function EvaluateTargetIfMongooseBite601(unit)
...
local function EvaluateTargetIfFilterMongooseBite657(unit)
  return Unit(unit):HasDeBuffsStacks(A.LatentPoisonDebuff.ID, true)
end

local function EvaluateTargetIfMongooseBite666(unit)
  return Unit(unit):HasDeBuffsStacks(A.LatentPoisonDebuff.ID, true) > 8
end


local function EvaluateTargetIfFilterKillCommand674(unit)
  return Unit(unit):DebuffRemainsP(A.BloodseekerDebuff.ID, true)
end

local function EvaluateTargetIfKillCommand687(unit)
  return Player:Focus() + Unit("player"):FocusCastRegen(A.KillCommand:GetSpellCastTime) < Player:FocusMax()
end


local function EvaluateTargetIfFilterSerpentSting725(unit)
  return Unit(unit):HasDeBuffs(A.SerpentStingDebuff.ID, true)
end

local function EvaluateTargetIfSerpentSting742(unit)
  return Unit("player"):HasBuffsStacks(A.VipersVenomBuff.ID, true)
end


local function EvaluateTargetIfFilterSerpentSting756(unit)
  return Unit(unit):HasDeBuffs(A.SerpentStingDebuff.ID, true)
end

local function EvaluateTargetIfSerpentSting787(unit)
...
local function EvaluateTargetIfFilterMongooseBite793(unit)
  return Unit(unit):HasDeBuffsStacks(A.LatentPoisonDebuff.ID, true)
end

local function EvaluateTargetIfFilterKillCommand828(unit)
  return Unit(unit):DebuffRemainsP(A.BloodseekerDebuff.ID, true)
end

local function EvaluateTargetIfKillCommand841(unit)
  return Player:Focus() + Unit("player"):FocusCastRegen(A.KillCommand:GetSpellCastTime) < Player:FocusMax()
end


local function EvaluateTargetIfFilterKillCommand1023(unit)
  return Unit(unit):DebuffRemainsP(A.BloodseekerDebuff.ID, true)
end

local function EvaluateTargetIfKillCommand1036(unit)
  ...
  S.MongooseBite = CurrentMongooseBite()
        ...
        local function Apst(unit)
            ...
            if A.KillCommand:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.KillCommand, 8, "min", EvaluateTargetIfFilterKillCommand47, EvaluateTargetIfKillCommand64) then 
            ...
            if A.KillCommand:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.KillCommand, 8, "min", EvaluateTargetIfFilterKillCommand126, EvaluateTargetIfKillCommand145) then 
                    return A.KillCommand:Show(icon) 
                end
            end
        ...
        local function Apwfi(unit)
            ...
            if A.KillCommand:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.KillCommand, 8, "min", EvaluateTargetIfFilterKillCommand247, EvaluateTargetIfKillCommand264) then 
                    return A.KillCommand:Show(icon) 
                end
            end
            -- steel_trap,if=focus+cast_regen<focus.max
            ...
            -- kill_command,target_if=min:bloodseeker.remains,if=focus+cast_regen<focus.max&(buff.mongoose_fury.stack<5|focus<action.mongoose_bite.cost)
            if A.KillCommand:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.KillCommand, 8, "min", EvaluateTargetIfFilterKillCommand300, EvaluateTargetIfKillCommand319) then 
                    return A.KillCommand:Show(icon) 
                end
            end
        ...
        local function Cleave(unit)
            ...
            if  then
                if Action.Utils.CastTargetIf(VarCarveCdr, 8, "min", EvaluateCycleCarveCdr542) then
            ...
            if A.MongooseBite:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.MongooseBite, 8, "min", EvaluateTargetIfFilterMongooseBite576, EvaluateTargetIfMongooseBite601) then 
                    return A.MongooseBite:Show(icon) 
                end
            end
            -- a_murder_of_crows
            ...
            if A.MongooseBite:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.MongooseBite, 8, "max", EvaluateTargetIfFilterMongooseBite657, EvaluateTargetIfMongooseBite666) then 
                    return A.MongooseBite:Show(icon) 
                end
            end
            -- chakrams
            ...
            if A.KillCommand:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.KillCommand, 8, "min", EvaluateTargetIfFilterKillCommand674, EvaluateTargetIfKillCommand687) then 
                    return A.KillCommand:Show(icon) 
                end
            end
            ...
            if A.SerpentSting:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.SerpentSting, 8, "min", EvaluateTargetIfFilterSerpentSting725, EvaluateTargetIfSerpentSting742) then 
                    return A.SerpentSting:Show(icon) 
                end
            end
            ...
            if A.SerpentSting:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.SerpentSting, 8, "min", EvaluateTargetIfFilterSerpentSting756, EvaluateTargetIfSerpentSting787) then 
                    return A.SerpentSting:Show(icon) 
                end
            end
            -- mongoose_bite,target_if=max:debuff.latent_poison.stack
            if A.MongooseBite:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.MongooseBite, 8, "max", EvaluateTargetIfFilterMongooseBite793) then 
                    return A.MongooseBite:Show(icon) 
                end
            end
        end
        ...
        local function St(unit)
            ...
            -- kill_command,target_if=min:bloodseeker.remains,if=focus+cast_regen<focus.max
            if A.KillCommand:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.KillCommand, 8, "min", EvaluateTargetIfFilterKillCommand828, EvaluateTargetIfKillCommand841) then 
                    return A.KillCommand:Show(icon) 
                end
            end
        ...
        local function Wfi(unit)
            ...
            if A.KillCommand:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.KillCommand, 8, "min", EvaluateTargetIfFilterKillCommand1023, EvaluateTargetIfKillCommand1036) then 
                    return A.KillCommand:Show(icon) 
                end
            end
            -- a_murder_of_crows
