...
local function EvaluateTargetIfFilterRisingSunKick453(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfRisingSunKick460(unit)
  return combo_strike
end


local function EvaluateTargetIfFilterFistoftheWhiteTiger468(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfFistoftheWhiteTiger475(unit)
  return Player:Chi() < 3
end


local function EvaluateTargetIfFilterBlackoutKick483(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfBlackoutKick492(unit)
  return combo_strike or not A.HitCombo:IsSpellLearned()
end


local function EvaluateTargetIfFilterRisingSunKick510(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfRisingSunKick521(unit)
  return A.TouchofDeath:GetCooldown() > 2 or VarHoldTod
end


local function EvaluateTargetIfFilterFistoftheWhiteTiger541(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfFistoftheWhiteTiger548(unit)
  return Player:Chi() < 3
end


local function EvaluateTargetIfFilterTigerPalm564(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfTigerPalm575(unit)
...
local function EvaluateTargetIfFilterBlackoutKick587(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfBlackoutKick612(unit)
...
local function EvaluateTargetIfFilterTigerPalm618(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfTigerPalm625(unit)
  return combo_strike and Player:ChiMax() - Player:Chi() > 1
end


local function EvaluateTargetIfFilterBlackoutKick633(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfBlackoutKick644(unit)
...
local function EvaluateTargetIfFilterFistoftheWhiteTiger684(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfFistoftheWhiteTiger707(unit)
...
local function EvaluateTargetIfFilterTigerPalm713(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfTigerPalm746(unit)
...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Serenity(unit)
            ...
            if A.RisingSunKick:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.RisingSunKick, 40, "min", EvaluateTargetIfFilterRisingSunKick453, EvaluateTargetIfRisingSunKick460) then 
                    return A.RisingSunKick:Show(icon) 
                end
            end
            ...
            if A.FistoftheWhiteTiger:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.FistoftheWhiteTiger, 40, "min", EvaluateTargetIfFilterFistoftheWhiteTiger468, EvaluateTargetIfFistoftheWhiteTiger475) then 
                    return A.FistoftheWhiteTiger:Show(icon) 
                end
            end
            ...
            if A.BlackoutKick:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.BlackoutKick, 40, "min", EvaluateTargetIfFilterBlackoutKick483, EvaluateTargetIfBlackoutKick492) then 
                    return A.BlackoutKick:Show(icon) 
                end
            end
            -- spinning_crane_kick
        ...
        local function St(unit)
            ...
            if A.RisingSunKick:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.RisingSunKick, 40, "min", EvaluateTargetIfFilterRisingSunKick510, EvaluateTargetIfRisingSunKick521) then 
                    return A.RisingSunKick:Show(icon) 
                end
            end
            ...
            -- fist_of_the_white_tiger,target_if=min:debuff.mark_of_the_crane.remains,if=chi<3
            if A.FistoftheWhiteTiger:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.FistoftheWhiteTiger, 40, "min", EvaluateTargetIfFilterFistoftheWhiteTiger541, EvaluateTargetIfFistoftheWhiteTiger548) then 
                    return A.FistoftheWhiteTiger:Show(icon) 
                end
            end
            ...
            if A.TigerPalm:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.TigerPalm, 40, "min", EvaluateTargetIfFilterTigerPalm564, EvaluateTargetIfTigerPalm575) then 
                    return A.TigerPalm:Show(icon) 
                end
            end
            -- chi_wave
            ...
            if A.BlackoutKick:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.BlackoutKick, 40, "min", EvaluateTargetIfFilterBlackoutKick587, EvaluateTargetIfBlackoutKick612) then 
                    return A.BlackoutKick:Show(icon) 
                end
            end
            -- tiger_palm,target_if=min:debuff.mark_of_the_crane.remains,if=combo_strike&chi.max-chi>1
            if A.TigerPalm:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.TigerPalm, 40, "min", EvaluateTargetIfFilterTigerPalm618, EvaluateTargetIfTigerPalm625) then 
                    return A.TigerPalm:Show(icon) 
                end
            end
            ...
            if A.BlackoutKick:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.BlackoutKick, 40, "min", EvaluateTargetIfFilterBlackoutKick633, EvaluateTargetIfBlackoutKick644) then 
                    return A.BlackoutKick:Show(icon) 
                end
            end
        end
        ...
        if inCombat and Unit(unit):IsExists() then
            ...
            if A.FistoftheWhiteTiger:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.FistoftheWhiteTiger, 40, "min", EvaluateTargetIfFilterFistoftheWhiteTiger684, EvaluateTargetIfFistoftheWhiteTiger707) then 
                    return A.FistoftheWhiteTiger:Show(icon) 
                end
            end
            ...
            if A.TigerPalm:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.TigerPalm, 40, "min", EvaluateTargetIfFilterTigerPalm713, EvaluateTargetIfTigerPalm746) then 
                    return A.TigerPalm:Show(icon) 
                end
            end
