...
Action[ACTION_CONST_ROGUE_ASSASSINATION] = {
    ...
    VendettaDebuff                         = Create({ Type = "Spell", ID = 79140 }),
    ShivDebuff                             = Create({ Type = "Spell", ID =  }),
    RuptureDebuff                          = Create({ Type = "Spell", ID = 1943 }),
    Subterfuge                             = Create({ Type = "Spell", ID = 108208 }),
    ShroudedSuffocation                    = Create({ Type = "Spell", ID =  }),
    GarroteDebuff                          = Create({ Type = "Spell", ID = 703 }),
    ...
    EchoingBlades                          = Create({ Type = "Spell", ID =  }),
    HiddenBladesBuff                       = Create({ Type = "Spell", ID =  }),
    DoubleDose                             = Create({ Type = "Spell", ID =  }),
    DeadlyPoisonDotDebuff                  = Create({ Type = "Spell", ID = 177918 }),
    EchoingReprimand                       = Create({ Type = "Spell", ID =  }),
    Ambush                                 = Create({ Type = "Spell", ID =  }),
    Mutilate                               = Create({ Type = "Spell", ID = 1329 }),
    ScentofBlood                           = Create({ Type = "Spell", ID =  }),
    Rupture                                = Create({ Type = "Spell", ID = 1943 }),
    CrimsonTempest                         = Create({ Type = "Spell", ID = 121411 }),
    CrimsonTempestBuff                     = Create({ Type = "Spell", ID = 121411 }),
    TwisttheKnife                          = Create({ Type = "Spell", ID =  }),
...
local function EvaluateTargetIfFilterMarkedForDeath43(unit)
  return Unit(unit):TimeToDie()
end

local function EvaluateTargetIfMarkedForDeath48(unit)
...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Precombat(unit)
            ...
            if A.GuardianofAzeroth:AutoHeartOfAzerothP(unit, true) and HeartOfAzeroth and (A.Exsanguinate:IsSpellLearned()) then
        ...
        local function Cds(unit)
            ...
            -- flagellation
            if A.Flagellation:IsReady(unit) then
            ...
            if A.AzsharasFontofPower:IsReady(unit) and (not Unit("player"):IsStealthed(true, true) and MasterAssassinRemains == 0 and num((A.Vendetta:GetCooldown() < ?{})) < 10 + 10 * num(A.AshvanesRazorCoral:IsExists()) and not Unit(unit):HasDeBuffs(A.VendettaDebuff.ID, true) and not Unit(unit):HasDeBuffs(A.ShivDebuff.ID, true)) then
                return A.AzsharasFontofPower:Show(icon)
            end
            ...
            if A.MarkedForDeath:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.MarkedForDeath, 40, "min", EvaluateTargetIfFilterMarkedForDeath43, EvaluateTargetIfMarkedForDeath48) then 
                    return A.MarkedForDeath:Show(icon) 
                end
            end
            ...
            VarVendettaFontCondition = num(not A.AzsharasFontofPower:IsExists() or A.ShroudedSuffocation:GetAzeriteRank() > 0 or Unit(unit):HasDeBuffsDown(A.RazorCoralDebuff.ID, true) or trinket.ashvanes_razor_coral.cooldown.remains < 10 and (A.Shiv:GetCooldown() < 1 or Unit(unit):HasDeBuffs(A.ShivDebuff.ID, true)))
            ...
            if A.Vanish:IsReady(unit) and ((A.MasterAssassin:IsSpellLearned() or runeforge.mark_of_the_master_assassin.equipped) and not Unit("player"):IsStealthed(true, true) and MasterAssassinRemains <= 0 and not Unit(unit):HasDeBuffsRefreshable(A.RuptureDebuff.ID, true) and Unit(unit):HasDeBuffs(A.GarroteDebuff.ID, true) > 3 and (Unit(unit):HasDeBuffs(A.VendettaDebuff.ID, true) and Unit(unit):HasDeBuffs(A.ShivDebuff.ID, true) and (not Azerite:EssenceHasMajor(A.BloodoftheEnemy.ID) or Unit(unit):HasDeBuffs(A.BloodoftheEnemyDebuff.ID, true)) or A.VisionofPerfection:IsSpellLearned())) then
                return A.Vanish:Show(icon)
            end
            ...
            if A.Shadowmeld:AutoRacial(unit) and Racial and A.BurstIsON(unit) and (not Unit("player"):IsStealthed(true, true) and A.ShroudedSuffocation:GetAzeriteRank() > 0 and Unit(unit):HasDeBuffsRefreshable(A.GarroteDebuff.ID, true) and A.PMultiplier(unit, A.GarroteDebuff.ID) <= 1 and Player:ComboPointsDeficit() >= 1) then
            ...
            if A.Shiv:IsReady(unit) and (Unit("player"):level >= 58 and Unit(unit):HasDeBuffs(A.RuptureDebuff.ID, true) and (not A.AzsharasFontofPower:IsExists() or A.Vendetta:GetCooldown() > 10)) then
                return A.Shiv:Show(icon)
            end
            
            -- potion,if=buff.bloodlust.react|debuff.vendetta.up
            if A.PotionofSpectralAgility:IsReady(unit) and Potion and (Unit("player"):HasHeroism() or Unit(unit):HasDeBuffs(A.VendettaDebuff.ID, true)) then
                return A.PotionofSpectralAgility:Show(icon)
            end
            
            -- blood_fury,if=debuff.vendetta.up
            if A.BloodFury:AutoRacial(unit) and Racial and A.BurstIsON(unit) and (Unit(unit):HasDeBuffs(A.VendettaDebuff.ID, true)) then
                return A.BloodFury:Show(icon)
            end
            
            -- berserking,if=debuff.vendetta.up
            if A.Berserking:AutoRacial(unit) and Racial and A.BurstIsON(unit) and (Unit(unit):HasDeBuffs(A.VendettaDebuff.ID, true)) then
                return A.Berserking:Show(icon)
            end
            
            -- fireblood,if=debuff.vendetta.up
            if A.Fireblood:AutoRacial(unit) and Racial and A.BurstIsON(unit) and (Unit(unit):HasDeBuffs(A.VendettaDebuff.ID, true)) then
                return A.Fireblood:Show(icon)
            end
            
            -- ancestral_call,if=debuff.vendetta.up
            if A.AncestralCall:AutoRacial(unit) and Racial and A.BurstIsON(unit) and (Unit(unit):HasDeBuffs(A.VendettaDebuff.ID, true)) then
        ...
        local function Direct(unit)
            ...
            if A.Envenom:IsReady(unit) and ((Player:ComboPoints() >= 4 + num(A.DeeperStratagem:IsSpellLearned()) or Player:ComboPoints() == animacharged_cp) and (Unit(unit):HasDeBuffs(A.VendettaDebuff.ID, true) or Unit(unit):HasDeBuffs(A.ShivDebuff.ID, true) or Player:EnergyDeficitPredicted() <= 25 + VarEnergyRegenCombined or not VarSingleUnit(unit)) and (not A.Exsanguinate:IsSpellLearned() or A.Exsanguinate:GetCooldown() > 2)) then
            ...
            if A.EchoingReprimand:IsReady(unit) and (VarUseFiller) then
                return A.EchoingReprimand:Show(icon)
            end
            
            -- ambush,if=variable.use_filler
            if A.Ambush:IsReady(unit) and (VarUseFiller) then
        ...
        local function Dot(unit)
            ...
            VarSkipCycleRupture = num(priority_rotation and MultiUnits:GetByRangeInCombat(10, 5, 10) > 3 and (Unit(unit):HasDeBuffs(A.ShivDebuff.ID, true) or (PoisonedBleeds() > 5 and not A.ScentofBlood:GetAzeriteRank() > 0)))
            
            -- variable,name=skip_rupture,value=debuff.vendetta.up&(debuff.shiv.up|master_assassin_remains>0)&dot.rupture.remains>2
            VarSkipRupture = num(Unit(unit):HasDeBuffs(A.VendettaDebuff.ID, true) and (Unit(unit):HasDeBuffs(A.ShivDebuff.ID, true) or MasterAssassinRemains > 0) and Unit(unit):HasDeBuffs(A.RuptureDebuff.ID, true) > 2)
            ...
            if A.CrimsonTempest:IsReady(unit) and (MultiUnits:GetByRangeInCombat(40, 5, 10) == 1 and Player:ComboPoints() >= (CPMaxSpend() - 1) and Unit("player"):HasBuffsRefreshable(A.CrimsonTempestBuff.ID, true) and not A.Exsanguinated(Unit(unit), "CrimsonTempest") and not Unit(unit):HasDeBuffs(A.ShivDebuff.ID, true) and MasterAssassinRemains == 0 and not A.TwisttheKnife:GetAzeriteRank() > 0 and Unit(unit):TimeToDie() - Unit("player"):HasBuffs(A.CrimsonTempestBuff.ID, true) > 4) then
                return A.CrimsonTempest:Show(icon)
            end
        ...
        local function Essences(unit)
            ...
            if A.ConcentratedFlame:AutoHeartOfAzerothP(unit, true) and HeartOfAzeroth and (Player:EnergyTimeToMaxPredicted() > 1 and not Unit(unit):HasDeBuffs(A.VendettaDebuff.ID, true) and (not Unit(unit):HasDeBuffs(A.ConcentratedFlameBurnDebuff.ID, true) and not A.ConcentratedFlame:IsSpellInFlight() or A.ConcentratedFlame:GetSpellChargesFullRechargeTime() < GetGCD())) then
            ...
            if A.BloodoftheEnemy:AutoHeartOfAzerothP(unit, true) and HeartOfAzeroth and (Unit(unit):HasDeBuffs(A.VendettaDebuff.ID, true) and (A.Exsanguinated(Unit(unit), "BloodoftheEnemy") or Unit(unit):HasDeBuffs(A.ShivDebuff.ID, true) and Player:ComboPointsDeficit() <= 1 or Unit(unit):HasDeBuffs(A.VendettaDebuff.ID, true) <= 10) or Unit(unit):TimeToDie() <= 10) then
                return A.BloodoftheEnemy:Show(icon)
            end
            
            -- guardian_of_azeroth,if=cooldown.vendetta.remains<3|debuff.vendetta.up|target.time_to_die<30
            if A.GuardianofAzeroth:AutoHeartOfAzerothP(unit, true) and HeartOfAzeroth and (A.Vendetta:GetCooldown() < 3 or Unit(unit):HasDeBuffs(A.VendettaDebuff.ID, true) or Unit(unit):TimeToDie() < 30) then
                return A.GuardianofAzeroth:Show(icon)
            end
            ...
            if A.GuardianofAzeroth:AutoHeartOfAzerothP(unit, true) and HeartOfAzeroth and (math.floor ((Unit(unit):TimeToDie() - 30) / cooldown) > math.floor ((Unit(unit):TimeToDie() - 30 - A.Vendetta:GetCooldown()) / cooldown)) then
                return A.GuardianofAzeroth:Show(icon)
            end
            
            -- focused_azerite_beam,if=spell_targets.fan_of_knives>=2|raid_event.adds.in>60&energy<70
            if A.FocusedAzeriteBeam:AutoHeartOfAzerothP(unit, true) and HeartOfAzeroth and (MultiUnits:GetByRangeInCombat(10, 5, 10) >= 2 or IncomingAddsIn > 60 and Player:EnergyPredicted() < 70) then
        ...
        local function Stealthed(unit)
            ...
            if A.CrimsonTempest:IsReady(unit) and (A.Nightstalker:IsSpellLearned() and MultiUnits:GetByRangeInCombat(40, 5, 10) >= 3 and Player:ComboPoints() >= 4 and Unit(unit):TimeToDie() - Unit("player"):HasBuffs(A.CrimsonTempestBuff.ID, true) > 6) then
                return A.CrimsonTempest:Show(icon)
            end
        ...
        if inCombat and Unit(unit):IsExists() then
            ...
            if A.SliceandDice:IsReady(unit) and (MultiUnits:GetByRangeInCombat(10, 5, 10) <= (5 - runeforge.dashing_scoundrel.equipped) and Unit("player"):HasBuffs(A.SliceandDiceBuff.ID, true) < fight_remains and Unit("player"):HasBuffs(A.SliceandDiceBuff.ID, true) < (1 + Player:ComboPoints()) * 1.8) then
                return A.SliceandDice:Show(icon)
            end
