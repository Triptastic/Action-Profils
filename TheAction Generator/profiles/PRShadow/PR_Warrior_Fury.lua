...
A:CreateCovenantsFor(ACTION_CONST_WARRIOR_FURY)  -- where PLAYERSPEC is Constance (example: ACTION_CONST_MONK_BM)
...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function SingleUnit(unit)(unit)
            ...
            if A.Rampage:IsReady(unit) and ((Unit("player"):HasBuffs(A.RecklessnessBuff.ID, true) or Unit("player"):HasBuffs(A.MemoryofLucidDreamsBuff.ID, true)) or (Unit("player"):HasBuffs(A.EnrageBuff.ID, true) < GetGCD() or Player:Rage() > 90)) then
            ...
            if A.Bloodbath:IsReady(unit) and (Unit("player"):HasBuffsDown(A.EnrageBuff.ID, true) or A.ColdSteelHotBlood:GetAzeriteRank() > 1) then
                return A.Bloodbath:Show(icon)
            end
            
            -- onslaught
            if A.Onslaught:IsReady(unit) then
            ...
            -- Crushing Blow,if=charges=2
            if A.Crushing Blow:IsReady(unit) and (A.Crushing Blow:GetSpellCharges() == 2) then
                return A.Crushing Blow:Show(icon)
            end
            ...
            -- Bloodbath
            if A.Bloodbath:IsReady(unit) then
                return A.Bloodbath:Show(icon)
            end
            
            -- raging_blow
            if A.RagingBlow:IsReady(unit) then
                return A.RagingBlow:Show(icon)
            end
            
            -- Crushing Blow
            if A.Crushing Blow:IsReady(unit) then
                return A.Crushing Blow:Show(icon)
            end
        ...
        if inCombat and Unit(unit):IsExists() then
            ...
            if A.Rampage:IsReady(unit) and (A.Recklessness:GetCooldown() < 3 and A.RecklessAbandon:IsSpellLearned()) then
                return A.Rampage:Show(icon)
            end
            ...
            if A.BloodoftheEnemy:AutoHeartOfAzerothP(unit, true) and HeartOfAzeroth and ((Unit("player"):HasBuffs(A.RecklessnessBuff.ID, true) or A.Recklessness:GetCooldown() < 1) and (Player:Rage() > 80 and (Unit("player"):HasBuffs(A.MeatCleaverBuff.ID, true) and Unit("player"):HasBuffs(A.EnrageBuff.ID, true) or MultiUnits:GetByRangeInCombat(8, 5, 10) == 1) or Unit(unit):HasDeBuffs(A.NoxiousVenomDebuff.ID, true))) then
            ...
            if A.Recklessness:IsReady(unit) and A.BurstIsON(unit) and (GetCurrentGCD() == 0 and (not Azerite:EssenceHasMajor(A.CondensedLifeforce.ID) and not Azerite:EssenceHasMajor(A.BloodoftheEnemy.ID) or A.GuardianofAzeroth:GetCooldown() > 1 or Unit("player"):HasBuffs(A.GuardianofAzerothBuff.ID, true) or Unit("player"):HasBuffs(A.BloodoftheEnemyBuff.ID, true))) then
                return A.Recklessness:Show(icon)
            end
