...
local function EvaluateTargetIfFilterRisingSunKick79(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfRisingSunKick94(unit)
...
local function EvaluateTargetIfFilterFistoftheWhiteTiger116(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfFistoftheWhiteTiger123(unit)
  return Player:ChiMax() - Player:Chi() >= 3
end


local function EvaluateTargetIfFilterTigerPalm129(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfTigerPalm138(unit)
  return Player:ChiMax() - Player:Chi() >= 2 and (not A.HitCombo:IsSpellLearned() or not combo_break)
end


local function EvaluateTargetIfFilterBlackoutKick150(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfBlackoutKick163(unit)
  return combo_strike and (Unit("player"):HasBuffs(A.BokProcBuff.ID, true) or (A.HitCombo:IsSpellLearned() and Player:PrevGCD(1, A.TigerPalm) and Player:Chi() < 4))
end


local function EvaluateTargetIfFilterRisingSunKick427(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfRisingSunKick434(unit)
  return combo_strike
end


local function EvaluateTargetIfFilterFistoftheWhiteTiger442(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfFistoftheWhiteTiger449(unit)
  return Player:Chi() < 3
end


local function EvaluateTargetIfFilterBlackoutKick457(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfBlackoutKick466(unit)
  return combo_strike or not A.HitCombo:IsSpellLearned()
end


local function EvaluateTargetIfFilterRisingSunKick484(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfRisingSunKick495(unit)
  return A.TouchofDeath:GetCooldown() > 2 or VarHoldTod
end


local function EvaluateTargetIfFilterFistoftheWhiteTiger515(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfFistoftheWhiteTiger522(unit)
  return Player:Chi() < 3
end


local function EvaluateTargetIfFilterTigerPalm538(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfTigerPalm549(unit)
...
local function EvaluateTargetIfFilterBlackoutKick561(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfBlackoutKick586(unit)
...
local function EvaluateTargetIfFilterTigerPalm592(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfTigerPalm599(unit)
  return combo_strike and Player:ChiMax() - Player:Chi() > 1
end


local function EvaluateTargetIfFilterBlackoutKick607(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfBlackoutKick618(unit)
  return (A.FistsofFury:GetCooldown() < 3 and Player:Chi() == 2 or Player:EnergyTimeToMaxPredicted() < 1) and (Player:PrevGCD(1, A.TigerPalm) or Player:ChiMax() - Player:Chi() < 2)
end


local function EvaluateTargetIfFilterFistoftheWhiteTiger658(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfFistoftheWhiteTiger681(unit)
...
local function EvaluateTargetIfFilterTigerPalm687(unit)
  return Unit(unit):HasDeBuffs(A.MarkoftheCraneDebuff.ID, true)
end

local function EvaluateTargetIfTigerPalm720(unit)
...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Aoe(unit)
            ...
            if A.EnergizingElixir:IsReady(unit) and A.BurstIsON(unit) and (not Player:PrevGCD(1, A.TigerPalm) and Player:Chi() <= 1 and Player:EnergyPredicted() < 50) then
            ...
            if A.RisingSunKick:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.RisingSunKick, 40, "min", EvaluateTargetIfFilterRisingSunKick79, EvaluateTargetIfRisingSunKick94) then 
            ...
            if A.FistoftheWhiteTiger:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.FistoftheWhiteTiger, 40, "min", EvaluateTargetIfFilterFistoftheWhiteTiger116, EvaluateTargetIfFistoftheWhiteTiger123) then 
            ...
            if A.TigerPalm:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.TigerPalm, 40, "min", EvaluateTargetIfFilterTigerPalm129, EvaluateTargetIfTigerPalm138) then 
            ...
            if A.BlackoutKick:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.BlackoutKick, 40, "min", EvaluateTargetIfFilterBlackoutKick150, EvaluateTargetIfBlackoutKick163) then 
        ...
        local function Serenity(unit)
            ...
            if A.RisingSunKick:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.RisingSunKick, 40, "min", EvaluateTargetIfFilterRisingSunKick427, EvaluateTargetIfRisingSunKick434) then 
                    return A.RisingSunKick:Show(icon) 
                end
            end
            ...
            if A.FistoftheWhiteTiger:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.FistoftheWhiteTiger, 40, "min", EvaluateTargetIfFilterFistoftheWhiteTiger442, EvaluateTargetIfFistoftheWhiteTiger449) then 
                    return A.FistoftheWhiteTiger:Show(icon) 
                end
            end
            ...
            if A.BlackoutKick:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.BlackoutKick, 40, "min", EvaluateTargetIfFilterBlackoutKick457, EvaluateTargetIfBlackoutKick466) then 
                    return A.BlackoutKick:Show(icon) 
                end
            end
            -- spinning_crane_kick
        ...
        local function St(unit)
            ...
            if A.RisingSunKick:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.RisingSunKick, 40, "min", EvaluateTargetIfFilterRisingSunKick484, EvaluateTargetIfRisingSunKick495) then 
                    return A.RisingSunKick:Show(icon) 
                end
            end
            ...
            -- fist_of_the_white_tiger,target_if=min:debuff.mark_of_the_crane.remains,if=chi<3
            if A.FistoftheWhiteTiger:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.FistoftheWhiteTiger, 40, "min", EvaluateTargetIfFilterFistoftheWhiteTiger515, EvaluateTargetIfFistoftheWhiteTiger522) then 
                    return A.FistoftheWhiteTiger:Show(icon) 
                end
            end
            ...
            if A.TigerPalm:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.TigerPalm, 40, "min", EvaluateTargetIfFilterTigerPalm538, EvaluateTargetIfTigerPalm549) then 
                    return A.TigerPalm:Show(icon) 
                end
            end
            -- chi_wave
            ...
            if A.BlackoutKick:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.BlackoutKick, 40, "min", EvaluateTargetIfFilterBlackoutKick561, EvaluateTargetIfBlackoutKick586) then 
                    return A.BlackoutKick:Show(icon) 
                end
            end
            -- tiger_palm,target_if=min:debuff.mark_of_the_crane.remains,if=combo_strike&chi.max-chi>1
            if A.TigerPalm:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.TigerPalm, 40, "min", EvaluateTargetIfFilterTigerPalm592, EvaluateTargetIfTigerPalm599) then 
                    return A.TigerPalm:Show(icon) 
                end
            end
            ...
            if A.BlackoutKick:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.BlackoutKick, 40, "min", EvaluateTargetIfFilterBlackoutKick607, EvaluateTargetIfBlackoutKick618) then 
                    return A.BlackoutKick:Show(icon) 
                end
            end
        end
        ...
        if inCombat and Unit(unit):IsExists() then
            ...
            if A.ProlongedPower:IsReady(unit) and Potion and (Unit("player"):HasBuffs(A.SerenityBuff.ID, true) or Unit("player"):HasBuffs(A.StormEarthandFireBuff.ID, true) or (not A.Serenity:IsSpellLearned() and trinket.proc.agility.react) or Unit("player"):HasHeroism() or Unit(unit):TimeToDie() <= 60) then
                return A.ProlongedPower:Show(icon)
            end
            ...
            if A.FistoftheWhiteTiger:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.FistoftheWhiteTiger, 40, "min", EvaluateTargetIfFilterFistoftheWhiteTiger658, EvaluateTargetIfFistoftheWhiteTiger681) then 
                    return A.FistoftheWhiteTiger:Show(icon) 
                end
            end
            ...
            if A.TigerPalm:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.TigerPalm, 40, "min", EvaluateTargetIfFilterTigerPalm687, EvaluateTargetIfTigerPalm720) then 
                    return A.TigerPalm:Show(icon) 
                end
            end
            -- chi_wave,if=!talent.fist_of_the_white_tiger.enabled&prev_gcd.1.tiger_palm&time<=3
            if A.ChiWave:IsReady(unit) and (not A.FistoftheWhiteTiger:IsSpellLearned() and Player:PrevGCD(1, A.TigerPalm) and Unit("player"):CombatTime() <= 3) then
                return A.ChiWave:Show(icon)
            end
