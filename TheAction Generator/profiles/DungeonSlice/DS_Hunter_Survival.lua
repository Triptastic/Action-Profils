...
local function EvaluateTargetIfFilterKillCommand261(unit)
  return Unit(unit):DebuffRemainsP(A.BloodseekerDebuff.ID, true)
end

local function EvaluateTargetIfKillCommand278(unit)
...
local function EvaluateTargetIfFilterKillCommand320(unit)
  return Unit(unit):DebuffRemainsP(A.BloodseekerDebuff.ID, true)
end

local function EvaluateTargetIfKillCommand339(unit)
  return Player:Focus() + Unit("player"):FocusCastRegen(A.KillCommand:GetSpellCastTime) < Player:FocusMax() and (Unit("player"):HasBuffsStacks(A.MongooseFuryBuff.ID, true) < 5 or Player:Focus() < A.MongooseBite:GetSpellPowerCostCache())
end


local function EvaluateCycleCarveCdr564(unit)
    return (MultiUnits:GetByRangeInCombat(8, 5, 10) < 5) and (MultiUnits:GetByRangeInCombat(8, 5, 10) < 5)
end

local function EvaluateTargetIfFilterMongooseBite598(unit)
  return Unit(unit):TimeToDie()
end

local function EvaluateTargetIfMongooseBite623(unit)
...
local function EvaluateTargetIfFilterMongooseBite679(unit)
  return Unit(unit):HasDeBuffsStacks(A.LatentPoisonDebuff.ID, true)
end

local function EvaluateTargetIfMongooseBite688(unit)
  return Unit(unit):HasDeBuffsStacks(A.LatentPoisonDebuff.ID, true) > 8
end


local function EvaluateTargetIfFilterKillCommand696(unit)
  return Unit(unit):DebuffRemainsP(A.BloodseekerDebuff.ID, true)
end

local function EvaluateTargetIfKillCommand709(unit)
  return Player:Focus() + Unit("player"):FocusCastRegen(A.KillCommand:GetSpellCastTime) < Player:FocusMax()
end


local function EvaluateTargetIfFilterSerpentSting747(unit)
  return Unit(unit):HasDeBuffs(A.SerpentStingDebuff.ID, true)
end

local function EvaluateTargetIfSerpentSting764(unit)
  return Unit("player"):HasBuffsStacks(A.VipersVenomBuff.ID, true)
end


local function EvaluateTargetIfFilterSerpentSting778(unit)
  return Unit(unit):HasDeBuffs(A.SerpentStingDebuff.ID, true)
end

local function EvaluateTargetIfSerpentSting809(unit)
...
local function EvaluateTargetIfFilterMongooseBite815(unit)
  return Unit(unit):HasDeBuffsStacks(A.LatentPoisonDebuff.ID, true)
end

local function EvaluateTargetIfFilterRaptorStrike826(unit)
  return Unit(unit):HasDeBuffsStacks(A.LatentPoisonDebuff.ID, true)
end

local function EvaluateTargetIfFilterKillCommand869(unit)
  return Unit(unit):DebuffRemainsP(A.BloodseekerDebuff.ID, true)
end

local function EvaluateTargetIfKillCommand882(unit)
  return Player:Focus() + Unit("player"):FocusCastRegen(A.KillCommand:GetSpellCastTime) < Player:FocusMax()
end


local function EvaluateTargetIfFilterKillCommand1070(unit)
  return Unit(unit):DebuffRemainsP(A.BloodseekerDebuff.ID, true)
end

local function EvaluateTargetIfKillCommand1083(unit)
...
local function APL()
  ...
  S.MongooseBite = CurrentMongooseBite()
        ...
        local function Apwfi(unit)
            ...
            if A.KillCommand:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.KillCommand, 8, "min", EvaluateTargetIfFilterKillCommand261, EvaluateTargetIfKillCommand278) then 
                    return A.KillCommand:Show(icon) 
                end
            end
            -- steel_trap,if=focus+cast_regen<focus.max
            ...
            -- kill_command,target_if=min:bloodseeker.remains,if=focus+cast_regen<focus.max&(buff.mongoose_fury.stack<5|focus<action.mongoose_bite.cost)
            if A.KillCommand:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.KillCommand, 8, "min", EvaluateTargetIfFilterKillCommand320, EvaluateTargetIfKillCommand339) then 
                    return A.KillCommand:Show(icon) 
                end
            end
            -- raptor_strike
        ...
        local function Cleave(unit)
            ...
            if A.MongooseBite:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.MongooseBite, 8, "min", EvaluateTargetIfFilterMongooseBite598, EvaluateTargetIfMongooseBite623) then 
                    return A.MongooseBite:Show(icon) 
                end
            end
            -- a_murder_of_crows
            ...
            if A.MongooseBite:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.MongooseBite, 8, "max", EvaluateTargetIfFilterMongooseBite679, EvaluateTargetIfMongooseBite688) then 
                    return A.MongooseBite:Show(icon) 
                end
            end
            -- chakrams
            ...
            if A.KillCommand:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.KillCommand, 8, "min", EvaluateTargetIfFilterKillCommand696, EvaluateTargetIfKillCommand709) then 
                    return A.KillCommand:Show(icon) 
                end
            end
            ...
            if A.SerpentSting:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.SerpentSting, 8, "min", EvaluateTargetIfFilterSerpentSting747, EvaluateTargetIfSerpentSting764) then 
                    return A.SerpentSting:Show(icon) 
                end
            end
            ...
            if A.SerpentSting:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.SerpentSting, 8, "min", EvaluateTargetIfFilterSerpentSting778, EvaluateTargetIfSerpentSting809) then 
                    return A.SerpentSting:Show(icon) 
                end
            end
            -- mongoose_bite,target_if=max:debuff.latent_poison.stack
            if A.MongooseBite:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.MongooseBite, 8, "max", EvaluateTargetIfFilterMongooseBite815) then 
                    return A.MongooseBite:Show(icon) 
                end
            end
            -- raptor_strike,target_if=max:debuff.latent_poison.stack
            if A.RaptorStrike:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.RaptorStrike, 8, "max", EvaluateTargetIfFilterRaptorStrike826) then 
                    return A.RaptorStrike:Show(icon) 
                end
            end
        end
        ...
        local function St(unit)
            ...
            -- kill_command,target_if=min:bloodseeker.remains,if=focus+cast_regen<focus.max
            if A.KillCommand:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.KillCommand, 8, "min", EvaluateTargetIfFilterKillCommand869, EvaluateTargetIfKillCommand882) then 
                    return A.KillCommand:Show(icon) 
                end
            end
        ...
        local function Wfi(unit)
            ...
            if A.KillCommand:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.KillCommand, 8, "min", EvaluateTargetIfFilterKillCommand1070, EvaluateTargetIfKillCommand1083) then 
                    return A.KillCommand:Show(icon) 
                end
            end
            -- a_murder_of_crows
