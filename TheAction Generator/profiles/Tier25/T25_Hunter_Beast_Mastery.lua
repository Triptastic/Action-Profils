...
Action[ACTION_CONST_HUNTER_BEASTMASTERY] = {
    ...
    SpittingCobra                          = Create({ Type = "Spell", ID = 194407 }),
...
A:CreateCovenantsFor(ACTION_CONST_HUNTER_BEASTMASTERY)  -- where PLAYERSPEC is Constance (example: ACTION_CONST_MONK_BM)
...
local function EvaluateTargetIfFilterBarbedShot145(unit)
  return Unit(unit):HasDeBuffs(A.BarbedShotDebuff.ID, true)
end

local function EvaluateTargetIfBarbedShot156(unit)
  return Pet:HasBuffs(A.FrenzyBuff.ID, true) and Pet:HasBuffs(A.FrenzyBuff.ID, true) <= GetGCD()
end


local function EvaluateTargetIfFilterBarbedShot166(unit)
  return Unit(unit):HasDeBuffs(A.BarbedShotDebuff.ID, true)
end

local function EvaluateTargetIfBarbedShot179(unit)
  return A.BarbedShot:GetSpellChargesFullRechargeTime() < GetGCD() and A.BestialWrath:GetCooldown()
end


local function EvaluateTargetIfFilterBarbedShot217(unit)
  return Unit(unit):HasDeBuffs(A.BarbedShotDebuff.ID, true)
end

local function EvaluateTargetIfBarbedShot242(unit)
...
A[3] = function(icon, isMulti)
    ...
    local function EnemyRotation(unit)
        ...
        local function Cds(unit)
            ...
            if A.BloodoftheEnemy:AutoHeartOfAzerothP(unit, true) and HeartOfAzeroth and (Player:Focus() < Player:FocusMax() and (0 > 90 or not (MultiUnits:GetByRangeInCombat(40, 5, 10) > 1) or MultiUnits:GetByRangeInCombat(40, 5, 10) > 1)) then
        ...
        local function Cleave(unit)
            ...
            if A.BarbedShot:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.BarbedShot, 40, "min", EvaluateTargetIfFilterBarbedShot145, EvaluateTargetIfBarbedShot156) then 
            ...
            if A.BarbedShot:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.BarbedShot, 40, "min", EvaluateTargetIfFilterBarbedShot166, EvaluateTargetIfBarbedShot179) then 
                    return A.BarbedShot:Show(icon) 
                end
            end
            -- aspect_of_the_wild
            ...
            if A.BarbedShot:IsReady(unit) then
                if Action.Utils.CastTargetIf(A.BarbedShot, 40, "min", EvaluateTargetIfFilterBarbedShot217, EvaluateTargetIfBarbedShot242) then 
                    return A.BarbedShot:Show(icon) 
                end
            end
        ...
        local function St(unit)
            ...
            if A.BarbedShot:IsReady(unit) and (Pet:HasBuffs(A.FrenzyBuff.ID, true) and Pet:HasBuffs(A.FrenzyBuff.ID, true) < GetGCD() or A.BestialWrath:GetCooldown() and (A.BarbedShot:GetSpellChargesFullRechargeTime() < GetGCD() or A.PrimalInstincts:GetAzeriteRank() > 0 and A.AspectoftheWild:GetCooldown() < GetGCD())) then
                return A.BarbedShot:Show(icon)
            end
            ...
            if A.BestialWrath:IsReady(unit) and (A.OneWiththePack:IsSpellLearned() and Unit("player"):HasBuffs(A.BestialWrathBuff.ID, true) < GetGCD() or Unit("player"):HasBuffsDown(A.BestialWrathBuff.ID, true) and A.AspectoftheWild:GetCooldown() > 15 or Unit(unit):TimeToDie() < 15 + GetGCD()) then
                return A.BestialWrath:Show(icon)
            end
        ...
        if inCombat and Unit(unit):IsExists() then
            ...
            if A.GalecallersBoon:IsReady(unit) and (Unit("player"):HasBuffs(A.AspectoftheWildBuff.ID, true) > 10 or A.AspectoftheWild:GetCooldown() > 45 or Unit(unit):TimeToDie() < 11) then
